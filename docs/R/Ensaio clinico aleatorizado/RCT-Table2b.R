TABLE.2b <- function(dataset, variables, covariate, bw.factor, control.g, wt.labels,
    ...
    mix.mod.res <- matrix("", nrow = length(t.labels), ncol = nlevels(bw.factor) *
        length(wt.labels))
    ...
    interaction <- paste("F(", format(round(mod1.aov[4, 1], digits = 0), nsmall = 0),
        ",", format(round(mod1.aov[4, 2], digits = 0), nsmall = 0), ")=", format(round(mod1.aov[4,
            3], digits = 3), nsmall = 3), ", p", p.value, flag, sep = "")
    ...
    for (i in 2:3) {
        ...
        model.res[i - 1, ] <- paste("F(", format(round(mod1.aov[i, 1], digits = 0),
            nsmall = 0), ",", format(round(mod1.aov[i, 2], digits = 0), nsmall = 0),
            ")=", format(round(mod1.aov[i, 3], digits = 3), nsmall = 3), ", p", p.value,
            flag, sep = "")
    }
    ...
    for (i in 1:(length(wt.labels))) {
        ...
        ifelse(p.values > alpha, bw.pvalues <- c(bw.pvalues, format(round(p.values,
            digits = 3), nsmall = 3)), "")
    ...
    bw.diff[1, ] <- wt.labels
    bw.diff[2, ] <- rep(paste(levels(bw.factor)[2], levels(bw.factor)[1], sep = " - "),
        length(wt.labels))
    bw.diff[4, ] <- bw
    ...
    print(paste("Table 2b: Two-way linear mixed model analysis (", toString(variables),
        ").", sep = ""), quote = FALSE)
