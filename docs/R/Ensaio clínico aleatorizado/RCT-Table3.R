TABLE.3 <-
  function(dataset,
    ...
    # control.g: a character label for the control group in ridit analysis
    ...
    cross.tab.res <-
      matrix("",
             ...
             ncol = nlevels(bw.factor) * length(c.labels))
    rownames(cross.tab.res) <- t.labels
    colnames(cross.tab.res) <-
      rep("", nlevels(bw.factor) * length(c.labels))
    ...
    for (i in 1:length(wt.labels)) {
      ...
      for (j in 1:nlevels(bw.factor)) {
        N <-
          table(factor(dataset[bw.factor == levels(bw.factor)[j], i], levels = c.labels))
        N.perc <-
          format(round(N / sum(N) * 100, digits = n.digits), nsmall = n.digits)
        desfecho <-
          cbind(desfecho, t(paste(N, " (", N.perc, "%)", sep = "")))
      }
      # ridit analysis
      r1 <-
        meanridits(table(dataset[, i], bw.factor),
                   margin = 2,
                   ref = control.g)[levels(bw.factor) == control.g]
      r2 <-
        meanridits(table(dataset[, i], bw.factor),
                   margin = 2,
                   ref = control.g)[levels(bw.factor) != control.g]
      se.diff <-
        seriditdiff(table(dataset[, i], bw.factor)[levels(bw.factor) == control.g], table(dataset[, i], bw.factor)[levels(bw.factor) !=
                                                                                                                     control.g])
