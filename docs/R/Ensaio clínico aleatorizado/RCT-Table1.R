TABLE.1 <-
  function(dataset,
    ...
    colnames(descript.res) <-
      c("Levels", paste("Group_", levels(bw.factor), sep = ""), "P-value")
    ...
    for (i in 1:length(variables)) {
      ...
      if (class(dataset[, i]) == 'character') {
        ...
        # compara os grupos
        p.value <-
          chisq.test(factor(dataset[, i], exclude = NULL), bw.factor, correct = TRUE)$p.value
        flag <- ""
        if (p.value < alpha) {
          flag <- "*"
        }
        ...
        tempdata <-
          cbind(levels(factor(dataset[, i], exclude = NULL)),
        ...
        rownames(tempdata) <-
          c(colnames(dataset)[i], rep("", nlevels(factor(
            dataset[, i], exclude = NULL
          )) - 1))
        ...
        # calcula os parametros descritivos
        descript.res[i, 2:(1 + nlevels(bw.factor))] <-
          paste(
            format(round(
        ...
        # compara os grupos
        p.value <-
          anova(lm(as.numeric(dataset[, i]) ~ bw.factor))[[5]][1]
        flag <- ""
        if (p.value < alpha) {
          flag <- "*"
        }
        ...
        # preenche a tabela de resultados
        descript.res[i, dim(descript.res)[2]] <-
          paste(format(round(p.value, digits = n.digits), nsmall = n.digits), flag, sep = "")
      }
    }
    
    # remove as variaveis nao analisadas
    descript.res <-
      descript.res[!apply(descript.res == "", 1, all),]
