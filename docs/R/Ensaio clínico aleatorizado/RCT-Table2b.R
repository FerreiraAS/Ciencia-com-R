TABLE.2b <-
  function(dataset,
    ...
    bw.diff <-
      matrix("", nrow = length(t.labels) + 1, ncol = (length(wt.labels)))
    ...
    mult.within <-
      summary(pairs(emmeans(mod1, ~ TIME_M |
                              GROUP_M), reverse = FALSE), infer = c(TRUE,
                                                                    TRUE))
    wt <- c()
    wt.pvalues <- c()
    ...
    smd.values <- c()
    mult.between <-
      summary(pairs(emmeans(mod1, ~ GROUP_M |
                              TIME_M), reverse = FALSE), infer = c(TRUE,
                                                                   TRUE))
    for (i in 1:(length(wt.labels))) {
      ...
      p.values <- as.numeric(group.data[, 9])
      ifelse(p.values < 0.001, bw.pvalues <-
               c(bw.pvalues, "<0.001*"), "")
      ifelse(p.values > 0.001 &
             ...
             "")
      ifelse(p.values > alpha, bw.pvalues <-
               c(bw.pvalues, format(round(
                 p.values, digits = 3
               ), nsmall = 3)),
             "")
    ...
    bw.diff[1,] <- wt.labels
    bw.diff[2,] <-
      rep(paste(levels(bw.factor)[2], levels(bw.factor)[1], sep = " - "), length(wt.labels))
    bw.diff[4,] <- bw
