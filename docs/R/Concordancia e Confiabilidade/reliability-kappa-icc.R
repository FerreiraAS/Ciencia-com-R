...
rater1 <- ordered(rater1, levels = c("Alta qualidade", "Moderada qualidade", "Baixa qualidade"))
rater2 <- data[which(raters.data$Examinador == names[2])]
rater2 <- ordered(rater2, levels = c("Alta qualidade", "Moderada qualidade", "Baixa qualidade"))

baseline <- raters.data[, 5]
baseline1 <- baseline[which(raters.data$Examinador == names[1])]
signal1 <- rowMeans(raters.data[which(raters.data$Examinador == names[1]), c(9, 19,
    29)])
...
signal2 <- rowMeans(raters.data[which(raters.data$Examinador == names[1]), c(9, 19,
    29)])
...
print("Table 1: Cross-table of qualitative assessment between raters.", quote = FALSE)
print(matrix(paste(table(rater1, rater2), " (", round(table(rater1, rater2)/length(ID) *
    100, digits = 1), "%)", sep = ""), nrow = 3, ncol = 3, dimnames = list(levels(rater1),
    levels(rater1))), quote = FALSE)
K <- cohen.kappa(cbind(rater1, rater2), alpha = 0.05)
print(paste("Cohen Kappa (unweighted) = ", round(K[[8]][1, 2], digits = 3), ", 95%CI = [",
    round(K[[8]][1, 1], digits = 3), "; ", round(K[[8]][1, 3], digits = 3), "]",
    sep = ""), quote = FALSE)
...
for (i in 1:n.raters) {
    ...
    for (k in 2:9) {
        ...
        temp <- cbind(data[which(data$Examinador == names[i]), k], data[which(data$Examinador ==
            names[i]), k + 10], data[which(data$Examinador == names[i]), k + 20])
        colnames(temp) <- c(colnames(data)[k], colnames(data)[k + 10], colnames(data)[k +
            20])

        # calculate ICC model and print outputs
        print("Table 2: Intrarater (test-retest) assessment.", quote = FALSE)
        print(describe(temp, interp = FALSE, skew = FALSE, ranges = FALSE, trim = 0.1,
            type = 3, check = TRUE, IQR = FALSE, omit = FALSE), quote = FALSE)
...
# drop empty column
inter.df <- inter.df[, -1]
variables <- sapply(strsplit(colnames(data)[2:9], split = "_AP", fixed = TRUE), function(x) (x[1]))
colnames(inter.df) <- paste(rep(variables, 2), c(rep(1, length(variables)), rep(2,
    length(variables))))
...
labels <- c("Rater #1, Mean (SD)", "Rater #2, Mean (SD)", "P-value", "Omega-sqr")
results.summ <- matrix(NA, nrow = length(variables), ncol = length(labels))
colnames(results.summ) <- labels
rownames(results.summ) <- variables
for (i in 1:length(variables)) {
    # summary statistics
    results.summ[i, 1] <- paste(format(round(mean(inter.df[, i]), digits = 1), nsmall = 1),
        " (", format(round(sd(inter.df[, i]), digits = 1), nsmall = 1), ")", sep = "")
    results.summ[i, 2] <- paste(format(round(mean(inter.df[, i + length(variables)]),
        digits = 1), nsmall = 1), " (", format(round(sd(inter.df[, i + length(variables)]),
        digits = 1), nsmall = 1), ")", sep = "")

    # anova model
    Y <- c(inter.df[, i], inter.df[, i + length(variables)])
    X <- data$Examinador
    results.summ[i, 3] <- format(round(anova(lm(Y ~ X))[[5]][1], digits = 3), nsmall = 3)
    results.summ[i, 4] <- format(round(omega_sq(aov(lm(Y ~ X))), digits = 3), nsmall = 3)

    # icc separate data for analysis
    temp <- cbind(inter.df[, i], inter.df[, i + length(variables)])
    colnames(temp) <- paste(variables[i], seq(1:n.raters))
    print(paste("Variables: ", paste(colnames(temp), collapse = ", "), sep = ""),
        quote = FALSE)
