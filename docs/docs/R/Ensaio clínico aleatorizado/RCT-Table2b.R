TABLE.2b <- function(dataset, variables, covariate, bw.factor, control.g, wt.labels, missing = c("complete.cases", 
	"mean.imputation", "multiple.imputation"), m.imputations, alpha, n.digits) {
...
# n.digits: number of decimal places to be presented for continuous variables
	...
	mix.mod.res <- matrix("", nrow = length(t.labels), ncol = nlevels(bw.factor) * length(wt.labels))
	...
	if (missing != "multiple.imputation") {
		if (!is.null(covariate)) {
			mod1 <- lme(fixed = OUTCOME_M ~ TIME_M * GROUP_M + COVARIATE_M, random = ~1 | ID_M/TIME_M, 
				data = data_M)
		} else {
			mod1 <- lme(fixed = OUTCOME_M ~ TIME_M * GROUP_M, random = ~1 | ID_M/TIME_M, data = data_M)
		}
		mod1.aov <- anova(mod1)
	} else {
		...
		imp <- mice(data_M, pred = pred, method = "2l.pan", m = m.imputations, seed = 0, print = FALSE)
		if (!is.null(covariate)) {
			mod1 <- with(data = imp, lme(fixed = OUTCOME_M ~ TIME_M * GROUP_M + COVARIATE_M, random = ~1 | 
				ID_M/TIME_M))
			mod1.aov <- quiet(mi.anova(imp, formula = "OUTCOME_M ~ TIME_M * GROUP_M + COVARIATE_M"))
		} else {
			mod1 <- with(data = imp, lme(fixed = OUTCOME_M ~ TIME_M * GROUP_M, random = ~1 | ID_M/TIME_M))
	...
	for (i in 1:length(wt.labels)) {
		for (j in 1:nlevels(bw.factor)) {
			# dados válidos
			N <- rbind(N, (paste("(n = ", sum(!is.na(OUTCOME_ORIG[TIME_M == i & GROUP_M == levels(bw.factor)[j]])), 
				")", sep = "")))
			# média (DP)
			desfecho <- rbind(desfecho, paste(format(round(mean(OUTCOME_M[TIME_M == i & GROUP_M == 
				levels(bw.factor)[j]], na.rm = TRUE), digits = n.digits), nsmall = n.digits), " (", format(round(sd(OUTCOME_M[TIME_M == 
				i & GROUP_M == levels(bw.factor)[j]], na.rm = TRUE), digits = n.digits), nsmall = n.digits), 
				")", sep = ""))
		}
	}
	...
	interaction <- paste("F(", format(round(mod1.aov[4, 1], digits = 0), nsmall = 0), ",", format(round(mod1.aov[4, 
		2], digits = 0), nsmall = 0), ")=", format(round(mod1.aov[4, 3], digits = 3), nsmall = 3), 
		", p", p.value, flag, sep = "")
	...
	for (i in 2:3) {
		...
		model.res[i - 1, ] <- paste("F(", format(round(mod1.aov[i, 1], digits = 0), nsmall = 0), ",", 
			format(round(mod1.aov[i, 2], digits = 0), nsmall = 0), ")=", format(round(mod1.aov[i, 3], 
				digits = 3), nsmall = 3), ", p", p.value, flag, sep = "")
	}

	# calcula e preenche a subtabela WITHIN-GROUP (SAME LINEAR MIXED MODEL)
	mult.within <- summary(pairs(emmeans(mod1, ~TIME_M | GROUP_M), reverse = FALSE), infer = c(TRUE, 
		TRUE))
	wt <- c()
	wt.pvalues <- c()
	for (i in 1:nlevels(bw.factor)) {
		...
		for (k in 1:(length(wt.labels) - 1)) {
			...
			ifelse(p.values[k] > 0.001 & p.values[k] < alpha, wt.pvalues <- c(wt.pvalues, paste(format(round(p.values[k], 
				digits = 3), nsmall = 3), "*", sep = "")), "")
			ifelse(p.values[k] > alpha, wt.pvalues <- c(wt.pvalues, format(round(p.values[k], digits = 3), 
				nsmall = 3)), "")
		}
	}
	...
	bw.pvalues <- c()
	smd.values <- c()
	mult.between <- summary(pairs(emmeans(mod1, ~GROUP_M | TIME_M), reverse = FALSE), infer = c(TRUE, 
		TRUE))
	for (i in 1:(length(wt.labels))) {
		...
		low.ci <- -round(as.numeric(group.data[, 7]), digits = n.digits)
		upp.ci <- -round(as.numeric(group.data[, 6]), digits = n.digits)
		bw <- cbind(bw, t(paste(estimate, " (", low.ci, " to ", upp.ci, ")", sep = "")))
		p.values <- as.numeric(group.data[, 9])
		ifelse(p.values < 0.001, bw.pvalues <- c(bw.pvalues, "<0.001*"), "")
		ifelse(p.values > 0.001 & p.values < alpha, bw.pvalues <- c(bw.pvalues, paste(format(round(p.values, 
			digits = 3), nsmall = 3), "*", sep = "")), "")
		ifelse(p.values > alpha, bw.pvalues <- c(bw.pvalues, format(round(p.values, digits = 3), nsmall = 3)), 
			"")
		...
		smd.values <- rbind(smd.values, t(paste(estimate, " (", lower, " to ", upper, ")", sep = "")))
	...
	print(paste("Table 2b: Two-way linear mixed model analysis (", toString(variables), ").", sep = ""), 
		quote = FALSE)
	...
	print("SMD¹ = Standardized Mean Difference calculated from marginal estimates (Cohen's d).", quote = FALSE)
	print("", quote = FALSE)
