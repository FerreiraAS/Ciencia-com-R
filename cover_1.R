...
#  rect(xleft = 0 - 10,  ybottom = H - 1 - 1, xright = W + 10, ytop = H - 1 + 1, col = "black")
  text(x = W, y = H/2 + 2 - 1, labels = substitute(paste(bold("Ciência com   "))), pos = 2, cex = 6.5, col = "yellow")
  text(x = W, y = H/2 + 2 - 1, labels = substitute(paste(bold("R"))), pos = 2, cex = 6.5, col = "#3363B0")
  
  # book subtitle
  text(x = W, y = H/2 + 3 - 2.75, labels = substitute(paste(italic("Perguntas e respostas para pesquisadores e analistas de dados"))), pos = 2, cex = 1.5, col = "white")
