...
-   Quanto à informação: quantitativa e qualitativa.[@Ali2016; @vetter2017; @Dettori2018; @kaliyadan2019]

-   Quanto ao conteúdo: contínua (intervalo ou discreta) ou categórica (ordinal ou nominal).[@barkan2015; @Ali2016; @vetter2017; @Dettori2018; @kaliyadan2019]

-   Quanto à interpretação: dependente (desfecho); independente (preditora, covariável, confundidora, controle); moderadora (interação); mediadora; auxiliar; indicadora; latente.[@Ali2016; @vetter2017; @Dettori2018; @kaliyadan2019]

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *base*[@base] fornece a função `class` para identificar qual é o tipo do objeto.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *base*[@base] fornece as funções `as.numeric` e `as.character` para criar objetos numéricos e categóricos, respectivamente.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *base*[@base] fornece as funções `as.Date` e `as.logical` para criar objetos em formato de data e lógicos (VERDADEIRO, FALSO), respectivamente.
:::
...
-   Mesmo quando se está usando análises consideradas robustas para violações dessas suposições ou testes não paramétricos (que não assumem explicitamente termos de erro normalmente distribuídos), atender a essas questões pode melhorar os resultados das análises.[@osborne2010]
...
```{r assimetria-direita, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Transformações de variáveis com assimetria à direita (Original, Raiz quadrada, Log natural, Log10, Inversa).", out.width = "100%", fig.width=15, fig.height=4, dpi=300}
# Reprodutibilidade
base::set.seed(123)
n <- 2000L
...
```{r assimetria-esquerda, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Transformações de variáveis com assimetria à esquerda (Original, Reflexão + Raiz quadrada, Reflexão + Log natural, Reflexão + Log10, Reflexão + Inversa).", out.width = "100%", fig.width=15, fig.height=4, dpi=300}
# Reprodutibilidade
base::set.seed(123)
n <- 2000L
...
-   Transformação $z$ de Fisher \@ref(eq:fisher-z).[@REF]
...
-   Transformação de Box-Cox \@ref(eq:box-cox).[@box1964]
...
-   Transformação arco-seno \@ref(eq:arco-seno).[@osborne2010]
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *MASS*[@MASS] fornece a função `boxcox` para executar a transformação de Box-Cox.[@box1964]
:::
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *base*[@base] fornece a função `scale` para calcular automaticamente a padronização (média = 0, desvio padrão = 1).
:::
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *questionr*[@questionr] fornece a função `irec` para executar uma interface interativa para codificação de variáveis categóricas.
:::
...
```{r variaveis-indicadoras, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, tab.cap = "Tabela de variáveis indicadoras (*dummy variables*) criadas a partir de variáveis categóricas Sexo e Grupo."}
# Carregar pacotes
library(dplyr)
...
dados <- data.frame(
  id = 1:n,
  sexo = factor(
    sample(c("Masculino", "Feminino"), n, replace = TRUE)
  ),
  grupo = factor(
    sample(c("Controle", "Tratamento A", "Tratamento B"), n, replace = TRUE)
  )
)

# 2. Criar variáveis indicadoras (dummy)
dummies_sexo  <- model.matrix(~ sexo,  data = dados)[, -1, drop = FALSE]
dummies_grupo <- model.matrix(~ grupo, data = dados)[, -1, drop = FALSE]

# 3. Juntar tudo em um único data.frame
tabela_dummy <- cbind(
  dados,
  as.data.frame(dummies_sexo),
  as.data.frame(dummies_grupo)
)

# Renomear colunas para clareza
colnames(tabela_dummy) <- c(
  "ID",
  "Sexo",
  "Grupo",
  "Sexo_Feminino",
...
knitr::kable(
  ...
  kableExtra::row_spec(base::dim(tabela_dummy)[1], extra_css = "border-bottom: 1px solid")
```

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *stats*[@stats] fornece a função `model.matrix` para expandir variáveis categóricas em variáveis indicadoras.
:::
...
### Por que não se usam $k$ variáveis indicadoras para $k$ níveis?
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *base*[@base] fornece a função `as.factor` para converter uma variável em fator.
:::
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *base*[@base] fornece as funções `levels` e `nlevels` para listar os níveis e a quantidade deles em um fator.
:::
