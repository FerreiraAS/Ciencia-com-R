...
- Treinar um modelo significa resolver um problema matemático no qual um conjunto de observações (dados) é usado para ajustar um modelo.[@burger2026]

- Modelos de aprendizado de máquina buscam capturar tendências gerais dos dados, ignorando particularidades excessivas para evitar sobreajuste (*overfitting*).[@burger2026]
...
O pacote *fastml*[@fastml] fornece a função `train_models` para treinar algoritmos de aprendizado de máquina em dados de treinamento pré-processados.
:::
...
- Aprendizado supervisionado é um paradigma em que um conjunto de variáveis preditoras (entradas) é utilizado para prever uma variável resposta previamente rotulada, seja ela quantitativa (regressão) ou qualitativa (classificação).[@bzdok2018]

- O algoritmo aprende padrões gerais a partir de exemplos observados, guiado por um objetivo explícito de predição.[@bzdok2018]

- Durante o treinamento, o modelo ajusta seus parâmetros para minimizar uma função de erro que compara valores previstos com rótulos verdadeiros, buscando equilíbrio entre ajuste aos dados e capacidade de generalização.[@bzdok2018]

<br>

### O que é aprendizado não supervisionado?

- Aprendizado não supervisionado é um paradigma em que o modelo tenta identificar padrões ou estruturas ocultas nos dados sem rótulos ou desfechos pré-definidos.[@chang2026]

- Diferentemente do aprendizado supervisionado, que busca prever um desfecho conhecido, o aprendizado não supervisionado é frequentemente utilizado para gerar descobertas orientadas por dados.[@chang2026]

- Esses métodos permitem identificar estruturas, agrupamentos, relações multivariadas e padrões latentes que não haviam sido previamente formulados como hipóteses.[@chang2026]
...
- Formulação de uma pergunta científica validável.[@chang2026]

- Preparação e exploração dos dados.[@chang2026]

- Aplicação de múltiplos métodos de modelagem.[@chang2026]

- Validação dos resultados.[@chang2026]

- Comunicação e documentação dos achados.[@chang2026]

- O planejamento do processo deve ocorrer antes da análise, incluindo estratégias de validação e critérios para comparação dos modelos.[@chang2026]

<br>

### Qual é o fluxo de desenvolvimento de modelos de aprendizado de máquina?

- Preparação e limpeza dos dados.[@sidey-gibbons2019]

- Divisão em conjuntos de treinamento, validação e teste.[@sidey-gibbons2019]

- Treinamento do modelo.[@sidey-gibbons2019]

- Ajuste de hiperparâmetros.[@sidey-gibbons2019]

- Avaliação do desempenho do modelo.[@sidey-gibbons2019]

- Validação em dados independentes.[@sidey-gibbons2019]

<Br>

### O que são conjuntos de treinamento, validação e teste?

- Conjunto de treinamento e validação são subdivisões dos dados, permitindo avaliar o desempenho do modelo em dados não utilizados durante o ajuste dos parâmetros.[@sidey-gibbons2019]

- Conjunto de treinamento: usado para ajustar os parâmetros do modelo.[@sidey-gibbons2019]

- Conjunto de validação: usado para comparar modelos e ajustar hiperparâmetros.[@sidey-gibbons2019]

- Conjunto de teste: usado apenas para avaliar o desempenho final do modelo.[@sidey-gibbons2019]

<br>

### O que são hiperparâmetros?

- Parâmetros definidos antes do treinamento do modelo.[@sidey-gibbons2019]

- Hiperparâmetros não são aprendidos diretamente a partir dos dados.[@sidey-gibbons2019]
...
- Quando a variável resposta é categórica, um método comum para estimar probabilidades e realizar classificação é a regressão logística, que modela a probabilidade de pertencimento a uma classe em função de variáveis preditoras.[@lever2016]

- Diferentemente da regressão linear, que pode produzir valores fora do intervalo $[0,1]$, a regressão logística utiliza uma função sigmoide que transforma combinações lineares de preditores em probabilidades válidas.[@lever2016]

- A função logística relaciona a probabilidade $p$ ao logaritmo da razão de chances (log-odds), produzindo uma relação linear entre o log-odds e os preditores.[@lever2016]

- Essa formulação permite interpretar os coeficientes do modelo como efeitos aditivos sobre o log-odds do evento de interesse.[@lever2016]

- Uma vez estimados os parâmetros, a probabilidade prevista pode ser obtida pela transformação inversa, o que gera uma curva signmóide, adequada para modelar transições graduais entre classes.[@lever2016]
...
- Máquinas de Vetores de Suporte (SVM) são métodos supervisionados que buscam encontrar um hiperplano que maximize a margem entre classes, utilizando apenas os vetores de suporte — os pontos mais próximos da fronteira decisória.[@bzdok2018]

- O equilíbrio entre margem e penalização de erros é controlado pelo hiperparâmetro $C$, que regula o *trade-off* entre viés e variância.[@bzdok2018]

- Valores elevados de $C$ reduzem violações da margem, mas aumentam risco de *overfitting*; valores menores favorecem margens mais amplas e maior regularização.[@bzdok2018]
...
- O algoritmo *k-nearest neighbors* (kNN) classifica novos pontos com base na maioria de votos entre os $k$ vizinhos mais próximos.[@bzdok2018]

- O kNN não assume fronteira paramétrica específica, sendo considerado método não paramétrico.[@bzdok2018]

- O valor de $k$ atua como parâmetro de regularização: valores pequenos produzem fronteiras altamente flexíveis (alta variância), enquanto valores maiores produzem fronteiras mais suaves (maior viés).[@bzdok2018]

<br>

```{r knn, echo=FALSE, warning=FALSE, message=FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Classificação usando o algoritmo KNN.", fig.width=8, fig.height=6, out.width="100%"}
# reproduzibilidade
set.seed(123)

# Dados simulados com duas classes
df_knn <- data.frame(
  x = c(rnorm(70, 0, 0.8), rnorm(70, 3, 0.8)),
  y = c(rnorm(70, 0, 0.8), rnorm(70, 3, 0.8)),
  classe = factor(c(rep("Classe A", 70), rep("Classe B", 70)))
)

# Novo ponto a ser classificado
novo_ponto <- data.frame(
  x = 1.6,
  y = 1.7
)
...
# Calcular distância euclidiana até o novo ponto
df_knn$distancia <- sqrt(
  (df_knn$x - novo_ponto$x)^2 +
  (df_knn$y - novo_ponto$y)^2
)
...
# Classe prevista por maioria de votos
classe_prevista <- vizinhos |>
  dplyr::count(classe) |>
  dplyr::arrange(dplyr::desc(n)) |>
  dplyr::slice(1) |>
  dplyr::pull(classe)

novo_ponto$classe_prevista <- classe_prevista

# Raio até o k-ésimo vizinho
raio_k <- max(vizinhos$distancia)

# Gráfico
ggplot2::ggplot(df_knn, ggplot2::aes(x = x, y = y, color = classe)) +
  ggplot2::geom_point(size = 2.5, alpha = 0.65) +
  ggforce::geom_circle(
    data = novo_ponto,
    ggplot2::aes(x0 = x, y0 = y, r = raio_k),
    inherit.aes = FALSE,
    linetype = "dashed",
    color = "black"
  ) +
...
```{r arvore-decisao, echo=FALSE, warning=FALSE, message=FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Árvore de decisão para predizer depressão a partir de idade, tabagismo e sintomas."}
# Dados simulados
set.seed(123)
...
- Árvores de decisão são construídas por um processo iterativo de particionamento do espaço dos preditores, no qual os dados são divididos em subconjuntos cada vez mais homogêneos em relação ao desfecho.[@krzywinski2017]

- Em cada etapa, o algoritmo avalia todos os possíveis pontos de divisão de uma variável preditora e seleciona aquele que maximiza o ganho de informação, isto é, a redução da mistura de classes entre os subconjuntos resultantes.[@krzywinski2017]

- O processo de divisão continua recursivamente até que seja atingido um critério de parada, como número mínimo de observações por nó, profundidade máxima da árvore ou redução mínima do erro.[@krzywinski2017]

- Alternativamente, a árvore pode ser inicialmente construída em sua forma completa e posteriormente simplificada por poda (*pruning*), geralmente utilizando validação cruzada para remover ramos pouco informativos e reduzir o risco de *overfitting*.[@krzywinski2017]
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *correctR*[@correctR] fornece as funções `kfold_ttest`, `repkfold_ttest` e `resampled_ttest` para calcular estatística para comparação de modelos de aprendizado de máquina em amostras dependentes.
:::
...
- O *k-means clustering* é um algoritmo de aprendizado não supervisionado utilizado para particionar um conjunto de dados em $k$ grupos, de modo que observações dentro do mesmo grupo apresentem maior similaridade entre si do que em relação às observações de outros grupos.[@schubert2023]

- O método pode ser interpretado como um problema de quantização de dados, no qual se busca representar um conjunto de $N$ observações em um espaço vetorial $d$ d-dimensional por meio de $k$ centros ou centroides.[@schubert2023]

- O objetivo do algoritmo é minimizar a soma dos quadrados das distâncias entre cada ponto e o centroide do cluster ao qual pertence, conhecida como *within-cluster sum of squares* ($WCSS$) ou *sum of squared errors* ($SSE$).[@schubert2023]

<br>

```{r k-means, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.cap = "Aplicação do algoritmo k-means para identificar grupos em um conjunto de dados bidimensional.", fig.width=8, fig.height=6, out.width="100%"}
# Reprodutibilidade
set.seed(123)

# Dados simulados com três grupos
df_kmeans <- data.frame(
  x = c(rnorm(80, 0, 0.6), rnorm(80, 3, 0.6), rnorm(80, 1.5, 0.6)),
  y = c(rnorm(80, 0, 0.6), rnorm(80, 3, 0.6), rnorm(80, 4, 0.6))
)

# Ajuste do k-means
km <- stats::kmeans(df_kmeans, centers = 3, nstart = 25)

df_kmeans$cluster <- factor(km$cluster)

centroides <- as.data.frame(km$centers)
centroides$cluster <- factor(1:3)

# Gráfico
ggplot2::ggplot(df_kmeans, ggplot2::aes(x = x, y = y, color = cluster)) +
  ...
  ggplot2::geom_text(
    data = centroides,
    ggplot2::aes(x = x, y = y, label = "Centroide"),
    vjust = -1.2,
    size = 3.5,
    color = "black",
    inherit.aes = FALSE
...
### Existe um número ótimo de *clusters*?

- Diferentemente de muitos problemas de aprendizado supervisionado, não existe necessariamente um único número correto de *clusters.*[@schubert2023]

- A análise de *clusters* deve ser vista como uma técnica exploratória, na qual diferentes soluções podem revelar estruturas relevantes em diferentes níveis de granularidade.[@schubert2023]

- A escolha do número de *clusters* deve considerar não apenas critérios quantitativos, mas também interpretação substantiva e conhecimento do domínio.[@schubert2023]

<br>

### Como escolher o número de *clusters*?

- Um dos principais desafios na aplicação do algoritmo k-means é determinar o número apropriado de *clusters* $k$, uma vez que esse parâmetro precisa ser especificado antes da execução do algoritmo.[@schubert2023]

- O algoritmo busca minimizar a soma dos quadrados das distâncias entre cada observação e o centroide do cluster mais próximo, conhecida como soma dos quadrados dentro dos *clusters* (*within-cluster sum of squares*, $WCSS$ ou $SSE$).[@schubert2023]

- Como o valor da $SSE$ tende a diminuir conforme aumenta o número de *clusters*, torna-se necessário utilizar algum critério para identificar um valor de $k$ que represente uma boa estrutura nos dados.[@schubert2023]

<br>

### Por que o método do “cotovelo” pode ser problemático?

- O método do cotovelo (*elbow method*) consiste em plotar a $SSE$ em função do número de *clusters* e escolher o ponto em que ocorre uma mudança abrupta na inclinação da curva.[@schubert2023]

- A intuição é que, após certo número de *clusters*, o ganho em redução do erro passa a apresentar retornos decrescentes.[@schubert2023]

- Entretanto, curvas semelhantes podem surgir mesmo quando os dados não possuem estrutura de *clusters* real.[@schubert2023]

Em muitos conjuntos de dados reais, o ponto de “cotovelo” não é claramente identificável, o que torna a escolha subjetiva.[@schubert2023]

- Diferentes heurísticas para detectar automaticamente o cotovelo frequentemente produzem resultados inconsistentes ou dependentes da escala dos dados.[@schubert2023]
...
- A análise de componentes principais é uma técnica estatística amplamente utilizada para redução de dimensionalidade, para representar dados de alta dimensão por um conjunto menor de variáveis, preservando o máximo possível da variabilidade original.[@dyer2023]
...
- A análise de componentes principais produz uma base ortogonal que impõe uma geometria específica à representação dos dados, restringindo a forma como os dados podem ser reconstruídos a partir dos componentes principais.[@dyer2023]

- Embora os componentes principais descrevam de maneira eficiente a variabilidade dos dados, eles nem sempre correspondem aos fatores geradores subjacentes do fenômeno estudado.[@dyer2023]

- A análise de componentes principais pode ainda introduzir padrões artificiais, criando uma aparência de estrutura que não reflete necessariamente os processos reais de geração dos dados.[@dyer2023]
...
p_biplot <- ggplot2::ggplot(df_scores, ggplot2::aes(PC1, PC2)) +
  ...
  ggplot2::geom_segment(
    ...
    arrow = grid::arrow(length = ggplot2::unit(0.18, "inches")),
    linewidth = 0.9
  ) +
  ggplot2::geom_text(
    data = df_load,
...
- Distância Chebyshev \@ref(eq:chebyshev): máximo das diferenças absolutas entre as coordenadas dos pontos, representando a distância em um espaço onde o movimento é permitido em todas as direções.[@REF]
...
# Manhattan Distance
p_manhattan <- ggplot2::ggplot() +
  ggplot2::geom_segment(ggplot2::aes(x=x[1], y=x[2], xend=y[1], yend=x[2]),
               size=1.2, color="orange") +
  ggplot2::geom_segment(ggplot2::aes(x=y[1], y=x[2], xend=y[1], yend=y[2]),
               size=1.2, color="orange") +
  ggplot2::geom_point(ggplot2::aes(x=x[1], y=x[2]), size=4) +
  ggplot2::geom_point(ggplot2::aes(x=y[1], y=y[2]), size=4, color="red") +
  ggplot2::coord_fixed() +
  ggplot2::ggtitle("Manhattan Distance (L1)") +
  ggplot2::theme_minimal()

# Euclidean Distance
p_euclid <- ggplot2::ggplot() +
  ggplot2::geom_segment(ggplot2::aes(x=x[1], y=x[2], xend=y[1], yend=y[2]),
               size=1.2, color="steelblue") +
  ggplot2::geom_point(ggplot2::aes(x=x[1], y=x[2]), size=4) +
  ggplot2::geom_point(ggplot2::aes(x=y[1], y=y[2]), size=4, color="red") +
  ggplot2::coord_fixed() +
  ggplot2::ggtitle("Euclidean Distance") +
  ggplot2::theme_minimal()

#  Chebyshev Distance
p_cheb <- ggplot2::ggplot() +
  ggplot2::geom_segment(ggplot2::aes(x=x[1], y=x[2], xend=y[1], yend=x[2]),
               linetype="dashed") +
  ggplot2::geom_segment(ggplot2::aes(x=y[1], y=x[2], xend=y[1], yend=y[2]),
               linetype="dashed") +
  ggplot2::geom_segment(ggplot2::aes(x=x[1], y=x[2], xend=x[1], yend=y[2]),
               size=1.2, color="purple") +
  ggplot2::geom_point(ggplot2::aes(x=x[1], y=x[2]), size=4) +
  ggplot2::geom_point(ggplot2::aes(x=y[1], y=y[2]), size=4, color="red") +
  ggplot2::coord_fixed() +
  ggplot2::ggtitle("Chebyshev Distance (L∞)") +
  ggplot2::theme_minimal()

# Cosine Distance (Angle)
p_cos <- ggplot2::ggplot() +
  ggplot2::geom_segment(ggplot2::aes(x=0, y=0, xend=x[1], yend=x[2]),
               size=1.2, color="blue") +
  ggplot2::geom_segment(ggplot2::aes(x=0, y=0, xend=y[1], yend=y[2]),
               size=1.2, color="red") +
  ggplot2::coord_fixed() +
  ggplot2::ggtitle("Cosine Similarity (Angle)") +
  ggplot2::theme_minimal()
...
p_hamming <- ggplot2::ggplot(hamming_df, ggplot2::aes(x=pos, y=1)) +
  ggplot2::geom_text(ggplot2::aes(label=x), vjust=-1) +
  ggplot2::geom_text(ggplot2::aes(label=y), y=0.8, vjust=1) +
  ggplot2::geom_point(ggplot2::aes(color=diff), size=4) +
  ggplot2::scale_color_manual(values=c("black","red")) +
  ggplot2::ggtitle("Hamming Distance") +
  ggplot2::theme_void()
...
p_jaccard <- ggplot2::ggplot() +
  ggforce::geom_circle(ggplot2::aes(x0=0, y0=0, r=2), fill="orange", alpha=0.4) +
  ggforce::geom_circle(ggplot2::aes(x0=2, y0=0, r=2), fill="blue", alpha=0.4) +
  ggplot2::coord_fixed() +
  ggplot2::ggtitle("Jaccard Distance (Set Overlap)") +
  ggplot2::theme_void()
...
- A área sob a curva ROC (AUROC) como medida de discriminação.[@vancalster2025; @assel2025]

- Um gráfico de calibração para avaliar a concordância entre probabilidades estimadas e proporções observadas.[@vancalster2025; @assel2025]

- Uma medida de utilidade clínica como o *net benefit* por meio de *decision curve analysis*.[@vancalster2025; @assel2025]

- Um gráfico que mostre a distribuição das probabilidades preditas por categoria de desfecho.[@vancalster2025; @assel2025]

<br>

### Quais métricas são consideradas inadequadas?

- Todas as métricas puramente baseadas em classificação dependente de limiar são consideradas impróprias para limiares clinicamente relevantes, exceto quando o limiar é 0,5 ou igual à prevalência verdadeira.[@vancalster2025]

- Métricas derivadas da matriz de confusão não devem ser usadas isoladamente como critério principal de avaliação, pois dependem da escolha do limiar e não refletem diretamente a utilidade, especialmente em cenários com dados desbalanceados.[@vancalster2025]

- O F-score $F_{\beta}$ pode produzir classificações de modelos inconsistentes com a utilidade clínica, favorecendo modelos menos úteis em cenários reais de decisão médica.[@assel2025]

- O F-score $F_{\beta}$ exige a dicotomização das probabilidades preditas, o que é indesejável em modelos, já que a tomada de decisão frequentemente depende de uma faixa contínua de riscos e de limiares que variam entre indivíduos e contextos.[@assel2025]

- O F1-score ($F1$) é explicitamente desencorajado como métrica principal porque não tem um foco claro em desempenho estatístico e é uma medida inadequada dentro de sua própria categoria conceitual.[@vancalster2025]
...
- Para evitar estimativas otimistas de desempenho, modelos devem ser avaliados em dados que não foram utilizados durante o treinamento.[@sidey-gibbons2019]

- Essa separação entre dados de treinamento e validação permite estimar a capacidade de generalização do algoritmo em novos dados.[@sidey-gibbons2019]
...
- F-beta score ($F_{\beta}$) \@ref(eq:fbeta): Média harmônica ponderada entre precisão e revocação, onde $\beta < 1$ prioriza precisão ($PRE$) e $\beta > 1$ prioriza sensibilidade $SEN$ ou $REV$.[@assel2025]

<br>

\begin{equation}
(\#eq:fbeta)
F_{\beta} = \dfrac{(1+\beta^{2}) \times TP}{(1+\beta^{2}) \times TP + \beta^{2} \times FN + FP}
\end{equation}

<br>

- F1-score ($F1$) \@ref(eq:f1): Média harmônica entre precisão e revocação, balanceando ambos os aspectos do desempenho ($\beta = 1$).[@hicks2022]

<br>

\begin{equation}
(\#eq:f1)
F1 = 2 \times \dfrac{PRE \times REV}{PRE + REV}
\end{equation}

<br>

## *Baselines* em classificação

<br>

### O que são *baselines*?

- *Baselines* são modelos extremamente simples que ignoram as variáveis preditoras e produzem previsões com base apenas na distribuição das classes ou em estratégias aleatórias simples.[@megahed2024]

<br>

### Por que é necessário comparar classificadores com um *baseline*?

- A avaliação de um classificador requer a comparação com um nível mínimo de desempenho esperado, denominado *baseline*.[@megahed2024]

- O objetivo de um *baseline* é estabelecer um ponto de referência para determinar se um modelo preditivo realmente apresenta desempenho superior ao acaso ou a estratégias triviais.[@megahed2024]

- Sem essa referência, métricas aparentemente altas podem ser enganosas, especialmente em conjuntos de dados com desbalanceamento de classes.[@megahed2024]

<br>

### Quais são os principais classificadores *baseline*?

- *Baselines* baseados em aleatoriedade produzem distribuições de desempenho (por exemplo, acurácia média próxima de 0,5 em classificação binária), de modo que a superioridade de um modelo deve ser avaliada em relação à variabilidade esperada desses valores.[@megahed2024]

- Esses modelos ignoram completamente as variáveis preditoras e, portanto, representam estratégias mínimas de classificação.[@megahed2024]

- Predição aleatória uniforme (*uniform random*): cada classe é prevista com igual probabilidade, independentemente da distribuição observada nos dados.[@megahed2024]

- Predição aleatória proporcional (*proportional random*): as classes são previstas aleatoriamente, mas respeitando as proporções observadas no conjunto de treinamento.[@megahed2024]

- Predição da classe mais frequente (*most frequent*): o modelo sempre prevê a classe mais comum no conjunto de dados.[@megahed2024]

<br>

### Como o desbalanceamento de classes afeta o *baseline*?

- Em conjuntos de dados balanceados, os *baselines* uniforme e proporcional tendem a apresentar desempenho semelhante.[@megahed2024]

- Em dados desbalanceados, o *baseline* proporcional frequentemente apresenta maior acurácia e especificidade, mas sensibilidade muito baixa para a classe minoritária.[@megahed2024]

- O *baseline* da classe mais frequente pode apresentar acurácia elevada mesmo sem qualquer capacidade real de discriminação.[@megahed2024]

- Por isso, métricas devem sempre ser interpretadas em relação ao desempenho esperado de um *baseline* apropriado.[@megahed2024]

<br>

### Como usar *baselines* na avaliação de modelos?

- O desempenho de um classificador deve ser considerado relevante apenas quando supera claramente o desempenho esperado do *baseline*.[@megahed2024]

- Em dados desbalanceados, o *baseline* proporcional geralmente é o mais adequado, pois incorpora a distribuição real das classes.[@megahed2024]

- Em aplicações com eventos raros, pode ser apropriado utilizar o *baseline* da classe mais frequente como referência prática.[@megahed2024]

- Em alguns casos, pode-se avaliar a distribuição de desempenho do *baseline* por simulação para determinar limiares estatisticamente superiores ao acaso.[@megahed2024]
