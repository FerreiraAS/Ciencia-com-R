...
-   A lei dos números anômalos --- lei de Benford --- é uma distribuição de probabilidade que descreve a frequência de ocorrência dos dígitos significativos em muitos conjuntos de dados do mundo real, sendo mais evidente na distribuição do primeiro dígito \@ref(eq:benford).[@Benford1938]
...
-   A lei tende a ocorrer quando os dados abrangem várias ordens de magnitude, não há limites artificiais, os dados são heterogêneos e os processos são multiplicativos.[@Benford1938]
...
-   A lei não se aplica a todos os conjuntos de dados, sendo menos adequada quando os valores possuem intervalos restritos ou limites artificiais.[@hill1995]
...
-   O teorema central do limite \@ref(eq:central-limit-theorem) afirma que, para uma amostra aleatória de tamanho $n$ de uma população com valor esperado igual à média $E[\bar{X_{i}}] = \mu$ e variância $Var[\bar{X_{i}}]$ igual a $\sigma^{2}$, a distribuição amostral da média de uma variável $\bar{X}$ se aproxima de uma distribuição normal $N$ com média $\mu$ e variância $\sigma^{2}/n$ à medida que $n$ aumenta ($n \to \infty$):[@kwak2017]
