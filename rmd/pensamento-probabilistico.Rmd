...
-   O teorema central do limite é um teorema que afirma que, para uma amostra aleatória de tamanho $n$ de uma população com média $\mu$ e variância $\sigma^{2}$, a distribuição amostral da média $\bar{X}$ se aproxima de uma distribuição normal com média $\mu$ e variância $\sigma^{2}/n$ à medida que $n$ aumenta.[@kwak2017]

- O teorema central do limite sugere que se o tamanho da amostra $n$ for suficientemente grande, a distribuição amostral das médias obtidas utilizando reamostragem com substituição será aproximadamente normal, com média $\mu$ e variância $\sigma^{2}/n$, independentemente da distribuição da população.[@kwak2017]

<br>

```{r teoroma-central-limite, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = "asis", fig.fullwidth = TRUE, fig.cap = "Histogramas de uma variável aleatória com distribuição uniforme.", eval = FALSE}
library(dplyr)
options(scipen=999)
set.seed(2024)

# simula uma variável aleatória de uma populacao
N <- 100000
populacao <- data.frame(
  ID = seq(start = 1, end = N, by = 1),
  Variável = round(runif(N, min = 0, max = 50), digits = 0)
)

# histograma de densidade da populacao
histograma_pop <- ggplot2::ggplot(populacao, ggplot2::aes(x = Variável)) +
  ...
  ggplot2::labs(
    title = paste0("População (n = ", format(N, big.mark = "."), "), média = ", round(mean(populacao$Variável), digits = 1), ", desvio padrão = ", round(sd(populacao$Variável), digits = 1)),
    x = "Variável",
    y = "Densidade"
  ) +
  ggplot2::theme_bw() +
...
# sorteia m participantes com reposição B vezes
...
for(i in 1:1000){
  sub <- amostra[sample(x = nrow(amostra), size = 5, replace = TRUE), ]
  reamostra$res.5[i] <- mean(sub)
  sub <- amostra[sample(x = nrow(amostra), size = 50, replace = TRUE), ]
  reamostra$res.50[i] <- mean(sub)
  sub <- amostra[sample(x = nrow(amostra), size = 500, replace = TRUE), ]
  reamostra$res.500[i] <- mean(sub)
  sub <- amostra[sample(x = nrow(amostra), size = 5000, replace = TRUE), ]
  reamostra$res.5000[i] <- mean(sub)
}

# histograma da média das reamostras
...
#   booktabs = TRUE,
#   linesep = "",
#   caption = paste0("Ppopulação simulada com distribuição uniforme (n = ", n, ").")
# ) %>%
...
### Qual a relevância do teorema central do limite para a análise eatatística?

- Baseados no teorema do limite central, os testes paramétricos têm maior poder estatístico do que os testes não paramétricos, os quais não requerem suposições de distribuição de probabilidade.[@kwak2017]
