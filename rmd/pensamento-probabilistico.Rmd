...
```{r teoroma-central-limite, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = "asis", fig.fullwidth = TRUE, fig.cap = "Esquerda: Histogramas de uma variável aleatória com distribuição uniforme (N = 100). Direita: Histogramas da média de 1000 amostras de tamanhos 5, 50, 500 e 5000 tomadas da população com reposição e igual probabilidade."}
library(dplyr)
options(scipen = 999)
set.seed(2024)
...
N <- 100
populacao <- data.frame(ID = seq(start = 1, end = N, by = 1),
                        Variável = round(runif(N, min = 18, max = 65), digits = 0))

# histograma de densidade da populacao
histograma_pop <-
  ggplot2::ggplot(populacao, ggplot2::aes(x = Variável)) +
  ...
  ggplot2::labs(
    title = paste0(
      "População (N = ", format(N, big.mark = "."), ")",
      "\n",
      "média = ",
      round(mean(populacao$Variável), digits = 2),
      "\n",
      "desvio-padrão = ",
      round(sd(populacao$Variável), digits = 2)
    ),
    x = "Variável",
    y = "Densidade"
  ) +
...
for (i in 1:B) {
  sub <- sample(x = populacao$Variável,
                size = 5,
                replace = TRUE)
...
names(reamostra) <- c(
  paste0(
    ...
    "\n",
    "média = ",
    round(mean(reamostra$res.5), digits = 2),
    "\n",
    "desvio-padrão = ",
    round(sd(reamostra$res.5), digits = 2)
  ),
  paste0(
    "Amostra (n = 50)",
    "\n",
    "média = ",
    round(mean(reamostra$res.50), digits = 2),
    "\n",
    "desvio-padrão = ",
    round(sd(reamostra$res.50), digits = 2)
  ),
  paste0(
    "Amostra (n = 500)",
    "\n",
    "média = ",
    round(mean(reamostra$res.500), digits = 2),
    "\n",
    "desvio-padrão = ",
    round(sd(reamostra$res.500), digits = 2)
  ),
  paste0(
    "Amostra: (n = 5000)",
    "\n",
    "média = ",
    round(mean(reamostra$res.5000), digits = 2),
    "\n",
    "desvio-padrão = ",
    round(sd(reamostra$res.5000), digits = 2)
  )
)
...
```{r, eval = FALSE}
# simula o lançamento de 1 dado k vezes
lançamentos <- tidydice::roll_dice(
        times = 1000,
        rounds = 1,
        success = 4)
...
- Como o teorema central do limite determina a distribuição amostral $Z$ das médias com tamanho amostral suficientemente grande, a média pode ser padronizada para uma distribuição normal com média 0 e variância 1, $N(0,1)$:[@kwak2017]
