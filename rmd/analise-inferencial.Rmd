...
O pacote *infer*[@infer] fornece a função [*specify*](https://www.rdocumentation.org/packages/infer/versions/1.1.0/topics/specify) para especificar as variáveis ou a relação entre elas.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *infer*[@infer] fornece a função [*hypothesize*](https://www.rdocumentation.org/packages/infer/versions/1.1.0/topics/hypothesize) para especificar a hipótese nula.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *infer*[@infer] fornece a função [*generate*](https://www.rdocumentation.org/packages/infer/versions/1.1.0/topics/generate) para gerar reamostras, permutações ou simulações.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *infer*[@infer] fornece a função [*calculate*](https://www.rdocumentation.org/packages/infer/versions/1.1.0/topics/calculate) para calcular estatísticas sumárias.
:::
...
- Além do papel técnico, os testes de hipótese carregam uma dimensão interpretativa que molda como os pesquisadores comunicam descobertas.[@uyguntunç2023]
...
- Teste de significância da hipótese nula: verifica evidência contra $H_{0}$ usando P-valor.[@lakens2018]

- Teste de mínimos efeitos: testa se o efeito é pelo menos tão grande quanto um limiar de relevância (*smallest effect size of interest*, SESOI). Rejeitar $H_{0}$ sugere efeito grande o suficiente.[@lakens2018]

- Teste de superioridade: avalia se uma intervenção supera outra, podendo testar tanto diferença em relação a zero quanto diferença superior a um menor efeito de interesse (*smallest effect size of interest*, SESOI).[@lakens2018; @mazzolari2022]

- Teste de equivalência ou dois testes unicaudais (*Two One-Sided Tests*, TOST): avalia se a diferença entre intervenções está inteiramente dentro de margens de equivalência previamente definidas ($-\Delta$ e $\Delta$). Rejeitar $H_{0}$ sugere equivalência prática.[@lakens2018; @mazzolari2022]

- Teste de não-inferioridade: avalia se uma intervenção não é pior que uma intervenção de referência por mais do que uma margem previamente definida ($-\Delta_{NI}$).[@mazzolari2022]

<br>

```{r tipos-testes-hipoteses, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Representação gráfica de diferentes tipos de teste de hipóteses: superioridade, equivalência (TOST) e não-inferioridade. As áreas coloridas indicam as regiões de decisão para cada teste, enquanto as linhas verticais representam os limites críticos (efeito nulo, SESOI/margem de equivalência e margem de não-inferioridade)."}
# Parâmetros
delta <- 0.8      # SESOI / margem de equivalência
delta_NI <- -0.8  # margem de não-inferioridade
...
dados <- data.frame(
  ...
  estimativa = c(1.20, 0.10, -0.20),
  ic_inf = c(0.85, -0.45, -0.60),
  ic_sup = c(1.55, 0.45, 0.20)
)
...
areas <- data.frame(
  ...
  xmin = c(delta, -delta, delta_NI),
  xmax = c(2, delta, 2),
  ymin = c(0.65, 0.65, 0.65),
  ymax = c(1.35, 1.35, 1.35),
  regiao = c(
    "Região de superioridade",
    "Região de equivalência",
    "Região de não-inferioridade"
  )
)

# Linhas de referência
linhas <- data.frame(
  teste = rep(dados$teste, each = 3),
  xintercept = rep(c(0, delta, delta_NI), times = 3),
  linha = rep(c("Efeito nulo", "SESOI / +Δ", "-Δ / ΔNI"), times = 3)
)
...
ggplot2::ggplot() +
  ...
  ggplot2::geom_vline(
    data = linhas,
    ggplot2::aes(xintercept = xintercept, linetype = linha),
    linewidth = 0.7
  ) +
  ...
  ggplot2::geom_point(
    data = dados,
    ggplot2::aes(x = estimativa, y = 1),
    size = 3
  ) +
  ggplot2::facet_wrap(~ teste, ncol = 1) +
  ggplot2::coord_cartesian(xlim = c(-1.5, 1.8), ylim = c(0.6, 1.4)) +
  ggplot2::scale_fill_manual(
    values = c(
      "Região de superioridade" = "steelblue",
      "Região de equivalência" = "darkseagreen",
      "Região de não-inferioridade" = "darkseagreen"
    )
  ) +
  ...
  ggplot2::labs(
    x = "Diferença entre intervenções",
    y = NULL,
    fill = NULL,
    linetype = NULL
  ) +
...
- Teste unicaudal à direita avalia evidência para a hipótese alternativa $H_A:\mu > \mu_0$. Busca-se verificar se o parâmetro populacional é significativamente maior que o valor especificado pela hipótese nula.[@kaiser1960]

- Teste unicaudal à esquerda avalia evidência para a hipótese alternativa $H_A:\mu < \mu_0$. Busca-se verificar se o parâmetro populacional é significativamente menor que o valor especificado pela hipótese nula.[@kaiser1960]

- Nos testes unicaudais, toda a região crítica é concentrada em uma única cauda da distribuição. O nível de significância $\alpha$ é alocado integralmente nessa direção, aumentando o poder estatístico para detectar efeitos no sentido previsto, mas não permitindo concluir significância no sentido oposto.[@kaiser1960]
...
- Teste bicaudal avalia evidência para a hipótese alternativa $H_A:\mu \neq \mu_0$, investigando se o parâmetro populacional difere do valor especificado pela hipótese nula, independentemente da direção da diferença.[@kaiser1960]

- Nos testes bicaudais, o nível de significância $\alpha$ é dividido entre as duas caudas da distribuição, geralmente em $\alpha/2$ para cada lado. Essa abordagem é apropriada quando diferenças positivas e negativas são igualmente plausíveis.[@kaiser1960]
...
- Para inferência visual, recomenda-se a apresentação dos dados em gráficos com estimativas de tendência central e seu intervalo (preferencialmente intervalo de confiança no nível de significância $\alpha$ pré-estabelecido).[@cumming2005]
...
- Resultados "positivos" compreendem um P-valor dentro da zona crítica estatisticamente significativa (ex.: $P<0,05$ ou outro ponto de corte).[@greenhalgh1997a]

- Resultados "positivos" sugerem que os autores rejeitem a hipótese nula ($H_{0}$), confirmando assim sua hipótese científica.[@greenhalgh1997a]

- Resultados "negativos" ou inconclusivos compreendem um P-valor fora da zona crítica estatisticamente significativa (ex.: $P \geq 0,05$ ou outro ponto de corte).[@greenhalgh1997a]

- Resultados "negativos" sugerem que os autores não rejeitem a hipótese nula ($H_{0}$) porque o efeito observado é nulo (logo, "negativo"), ou porque o estudo não possui poder suficiente para detectá-lo, não permitindo afirmar a hipótese científica (logo, inconclusivo).[@greenhalgh1997a]
...
- Testes de hipótese tendem a reduzir a incerteza científica a decisões docotômicas.[@uyguntunç2023]

- Essa simplificação possui implicações epistemológicas importantes: os testes de hipótese produzem não apenas juízos empíricos, mas também atos pragmáticos que comunicam graus de confiança e orientam ações.[@uyguntunç2023]
