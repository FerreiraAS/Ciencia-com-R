...
ggplot2::ggplot(data.frame(p_values), ggplot2::aes(x = p_values)) +
  ...
  ggplot2::labs(
    title = "Distribuição dos p-valores em 100 testes de hipótese",
    subtitle = paste0("Número de p-valores menores que 0,05: ", p_less_than_0_05, "\n"),
       x = "P-valor",
       y = "Frequência") +
...
n_tests <- 100
n_samples <- 10  # Amostra pequena para aumentar a chance de erro tipo II
true_mean <- 0.5  # Valor real da média ≠ 0 (H₀ é falsa)

# Vetor para armazenar p-valores
p_values <- numeric(n_tests)

for (i in 1:n_tests) {
  # Gerar amostra de uma normal com média diferente de 0
...
ggplot2::ggplot(data.frame(p_values), ggplot2::aes(x = p_values)) +
  ...
  ggplot2::geom_vline(xintercept = 0.05, color = "red", linetype = "dashed", size = 1) +
  ggplot2::annotate("text", x = 0.25, y = 15, label = "p > 0,05 (erro tipo II)", color = "red", size = 5) +
  ggplot2::theme_minimal() +
  ggplot2::labs(
    title = "Distribuição dos p-valores em 100 testes com H₀ falsa",
    subtitle = paste0("Número de p-valores maiores que 0,05 (erro tipo II): ", p_greater_than_0_05, "\n"),
    x = "P-valor",
    y = "Frequência") +
  ggplot2::theme(axis.text = ggplot2::element_text(size = 12),
                 axis.title = ggplot2::element_text(size = 14))
