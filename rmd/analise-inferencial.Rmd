...
```{r analise-inferencial-unicaudal-plot, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = "asis", fig.fullwidth = TRUE, fig.cap = "Representação gráfica de um teste de hipótese (unicaudal)."}
...
plot(x, dens_H0, type = "l", lwd = 2, col = "blue",
     ylim = c(0, max(dens_H0)),
     ylab = "Densidade", xlab = "Efeito",
...
# Annotations
text(mu0, max(dens_H0) * 0.85, expression(mu[0]), col = "blue", pos = 4)
text(z_crit + 0.1, max(dens_H0) * 0.95, expression(z[1 - alpha]), col = "black", pos = 4)
...
```{r analise-inferencial-bicaudal-plot, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = "asis", fig.fullwidth = TRUE, fig.cap = "Representação gráfica de um teste de hipótese (bicaudal)."}
# Define parameters for H0
mu0 <- 0       # Mean under H0
sigma <- 1     # Standard deviation
alpha <- 0.05  # Significance level (total)
...
polygon(c(min(x), x[x <= z_crit_low], z_crit_low),
        c(0, dens_H0[x <= z_crit_low], 0),
        col = rgb(0, 0, 1, 0.3), border = NA)

# Shade right tail (Type I Error α/2)
polygon(c(z_crit_high, x[x >= z_crit_high], max(x)),
        c(0, dens_H0[x >= z_crit_high], 0),
        col = rgb(0, 0, 1, 0.3), border = NA)

# Add vertical lines
abline(v = mu0, col = "blue", lty = 3, lwd = 1.5)
abline(v = z_crit_low, col = "black", lty = 2, lwd = 1.5)
abline(v = z_crit_high, col = "black", lty = 2, lwd = 1.5)

# Annotations
text(mu0, max(dens_H0) * 0.85, expression(mu[0]), col = "blue", pos = 4)
text(z_crit_low - 0.1, max(dens_H0) * 0.95, expression(z[alpha/2]), col = "black", pos = 2)
text(z_crit_high + 0.1, max(dens_H0) * 0.95, expression(z[1 - alpha/2]), col = "black", pos = 4)

# Legend
legend("topleft",
       legend = c(expression("Distribuição H"[0]),
                  "Região crítica (α/2)",
                  expression("Valores críticos (" * z[alpha/2] * ", " * z[1 - alpha/2] * ")")),
       col = c("blue", rgb(0, 0, 1, 0.3), "black"),
       lwd = c(2, NA, 1.5),
       lty = c(1, NA, 2),
       pch = c(NA, 15, NA),
...
```{r erros-tipo-S-plot, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = "asis", fig.fullwidth = TRUE, fig.cap = "Representação gráfica do erro tipo S (sinal) em um teste de hipótese (bicaudal)."}

mu1 <- 0.2     # True effect (small positive)
sigma <- 1
alpha <- 0.05
n <- 30
se <- sigma / sqrt(n)
...
repeat {
  ...
  z <- (obs_mean - mu0) / se
  if (abs(z) > z_crit && obs_mean < 0) break  # Sign error: significant but negative
}
# x values
...
par(mar = c(5, 4, 4, 4))
plot(x, dens_H0, type = "l", lwd = 2, col = "blue", ylim = c(0, max(dens_H0, dens_H1)),
     ylab = "Densidade", xlab = "Efeito estimado", main = "Erro Tipo-S: Sinal Incorreto")
lines(x, dens_H1, lwd = 2, col = "red")
...
polygon(c(min(x), x[x <= x_lower], x_lower),
        ...
        col = rgb(1, 0, 0, 0.3), border = NA)
# Vertical lines
abline(v = x_lower, col = "black", lty = 2, lwd = 1.5)
abline(v = x_upper, col = "black", lty = 2, lwd = 1.5)
abline(v = mu0, col = "blue", lty = 3, lwd = 1.5)
abline(v = mu1, col = "red", lty = 3, lwd = 1.5)
abline(v = obs_mean, col = "darkgreen", lwd = 2)
# Labels
text(x_lower - 0.02, max(dens_H0) * 0.5, expression(alpha/2), col = "blue", pos = 2)
text(x_upper + 0.02, max(dens_H0) * 0.5, expression(alpha/2), col = "blue", pos = 4)
text(mu0, max(dens_H0) * 0.8, expression(mu[0]), col = "blue", pos = 4)
text(mu1, max(dens_H1) * 0.8, expression(mu[1]), col = "red", pos = 4)
text(obs_mean, max(dens_H1) * 0.9, "Estimado", col = "darkgreen", pos = 4)
# Legend
legend("topleft",
       legend = c(expression("H"[0]), expression("H"[1]),
                  "Erro tipo I (α)", "Erro tipo-S (sinal errado)",
                  "Estimativa significativa"),
       col = c("blue", "red", rgb(0, 0, 1, 0.3), rgb(1, 0, 0, 0.3), "darkgreen"),
       lwd = c(2, 2, NA, NA, 2),
       lty = c(1, 1, NA, NA, 1),
       pch = c(NA, NA, 15, 15, NA),
       pt.cex = c(NA, NA, 2, 2, NA),
...
```{r erros-tipo-M-plot, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = "asis", fig.fullwidth = TRUE, fig.cap = "Representação gráfica do erro tipo M (magnitude) em um teste de hipótese (bicaudal)."}

set.seed(123)

# Parameters
mu0 <- 0       
mu1 <- 0.2     # True effect (small)
sigma <- 1     
alpha <- 0.05  
n <- 30        
se <- sigma / sqrt(n)
...
par(mar = c(5, 4, 4, 4))
plot(x, dens_H0, type = "l", lwd = 2, col = "blue", ylim = c(0, max(dens_H0, dens_H1)),
     ylab = "Densidade", xlab = "Efeito estimado", main = "Erro Tipo-M: Superestimação do Efeito")
lines(x, dens_H1, lwd = 2, col = "red")
...
# Vertical lines
abline(v = x_crit, col = "black", lty = 2, lwd = 1.5)
abline(v = mu0, col = "blue", lty = 3, lwd = 1.5)
abline(v = mu1, col = "red", lty = 3, lwd = 1.5)
abline(v = obs_mean, col = "darkgreen", lwd = 2)

# Labels
text(x_crit + 0.02, max(dens_H0) * 0.5, expression(alpha), col = "blue", pos = 4)
text(mu0, max(dens_H0) * 0.8, expression(mu[0]), col = "blue", pos = 4)
text(mu1, max(dens_H1) * 0.8, expression(mu[1]), col = "red", pos = 4)
text(obs_mean, max(dens_H1) * 0.9, "Estimado", col = "darkgreen", pos = 4)
