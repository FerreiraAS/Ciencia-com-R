...
- Razão de verossimilhança positiva ($LR+$) \@ref(eq:lrplus): Quantifica o quanto a probabilidade de a condição estar presente aumenta quando o teste é positivo.[@REF]
...
- Razão de verossimilhança negativa ($LR-$) \@ref(eq:lrminus): Quantifica o quanto a probabilidade de a condição estar presente diminui quando o teste é negativo.[@REF]
...
knitr::kable(
  ...
  kableExtra::row_spec(0, bold = TRUE, extra_css = "border-top: 1px solid; border-bottom: 1px solid") |>
  kableExtra::row_spec(nrow(cross3_totais)-1, bold = TRUE, extra_css = "border-bottom: 1px solid") |>  # penúltima linha (antes do total) com borda
  kableExtra::column_spec(1, bold = TRUE) |>
...
### O que um gráfico de cruzes (*crosshair*)?

- .[@phillips2010]

<br>

```{r crosshair, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Gráfico de cruzes (*crosshair*) em espaço ROC (*Receiver Operating Characteristic*) para 15 estudos simulados de desempenho diagnóstico."}
# Reprodutibilidade
set.seed(123)

# ---- simulate 2x2 tables ----
n_studies <- 15
Nd <- sample(60:200, n_studies, replace = TRUE)
Nn <- sample(60:220, n_studies, replace = TRUE)

true_sens <- rbeta(n_studies, 20, 6)
true_spec <- rbeta(n_studies, 25, 8)

TP <- rbinom(n_studies, Nd, true_sens)
FN <- Nd - TP
TN <- rbinom(n_studies, Nn, true_spec)
FP <- Nn - TN

df <- data.frame(
  study = paste0("Estudo_", sprintf("%02d", seq_len(n_studies))),
  TP, FN, FP, TN,
  Nd, Nn
)

# ---- helpers: exact (Clopper–Pearson) CI via binom.test ----
binom_ci <- function(x, n, conf = 0.95) binom.test(x, n, conf.level = conf)$conf.int
...
df <- df %>%
  ...
  dplyr::mutate(
    ...
    spec = TN / (TN + FP),
    # exact CI
    sens_ci = list(binom_ci(TP, TP + FN)),
    spec_ci = list(binom_ci(TN, TN + FP))
  ) %>%
  dplyr::mutate(
    sens_low = sens_ci[[1]], sens_up = sens_ci[[2]],
    spec_low = spec_ci[[1]], spec_up = spec_ci[[2]]
  ) %>%
  dplyr::ungroup() %>%
  # ---- ROC space: FPR = 1 - Specificity ----
  dplyr::mutate(
    fpr     = 1 - spec,
    fpr_low = 1 - spec_up,   # invert bounds
    fpr_up  = 1 - spec_low,
    tot_n   = Nd + Nn,
    size_pt = scales::rescale(tot_n, to = c(2.5, 6))  # point size by N
  )

# ---- plot crosshair ----
p <- ggplot2::ggplot(df, ggplot2::aes(x = fpr, y = sens)) +
  ggplot2::geom_abline(
    slope = 1, intercept = 0,
    linetype = "dashed", colour = "grey50", linewidth = 0.6
  ) +
  ggplot2::geom_errorbar(
    ggplot2::aes(ymin = sens_low, ymax = sens_up),
    width = 0.008, alpha = 0.8
  ) +
  ggplot2::geom_errorbarh(
    ggplot2::aes(xmin = fpr_low, xmax = fpr_up),
    height = 0.008, alpha = 0.8
  ) +
  ggplot2::geom_point(
    ggplot2::aes(size = size_pt),
    shape = 21, fill = "gray20", color = "white"
  ) +
  ...
  ggplot2::theme(
    ...
    panel.border = ggplot2::element_rect(
      colour = "black", fill = NA, linewidth = 0.8
    ),

    # keep axis lines off (border replaces them)
