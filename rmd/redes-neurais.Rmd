...
-   Neurônios artificiais (ou perceptrons) são modelos matemáticos que imitam o funcionamento dos neurônios biológicos, recebendo entradas, aplicando pesos e uma função de ativação para produzir uma saída.[@mcculloch1943; @rosenblatt1958; @rosenblatt1960]
...
```{r rede-neural, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Esquema de diferentes arquiteturas de redes neurais artificiais.", fig.width=15, fig.height=10, out.width="100%"}
# CARREGAR PACOTES
library(patchwork)
...
# Criar camada de nós
create_layer <- function(n, x, type = "hidden") {
  ...
  data.frame(
    id = paste0(type, "_", x, "_", seq_len(n)),
    x = x,
    y = y_positions,
    type = type,
...
# Conectar duas camadas completamente
connect_layers <- function(layer1, layer2) {
  expand.grid(
    from = layer1$id,
    to = layer2$id,
    stringsAsFactors = FALSE
...
# Plot principal
plot_network <- function(layers, edges, title = "") {
  nodes <- do.call(rbind, layers)
...
# 1. PERCEPTRON
input_p <- create_layer(3, 1, "input")
output_p <- create_layer(1, 2, "output")

edges_p <- connect_layers(input_p, output_p)

p1 <- plot_network(
  list(input_p, output_p),
  edges_p,
  "Perceptron"
)

# 2. FEED FORWARD NETWORK
input_ff <- create_layer(3, 1, "input")
hidden_ff <- create_layer(4, 2, "hidden")
output_ff <- create_layer(2, 3, "output")

edges_ff <- rbind(
  connect_layers(input_ff, hidden_ff),
  connect_layers(hidden_ff, output_ff)
)

p2 <- plot_network(
  list(input_ff, hidden_ff, output_ff),
  edges_ff,
  "RN Feed Forward"
)

# 3. RECURRENT NEURAL NETWORK
input_rnn <- create_layer(3, 1, "input")
hidden_rnn <- create_layer(4, 2, "recurrent")
output_rnn <- create_layer(2, 3, "output")

edges_rnn <- rbind(
  connect_layers(input_rnn, hidden_rnn),
  connect_layers(hidden_rnn, output_rnn)
)

# conexões recorrentes
recurrent_edges <- data.frame(
  from = hidden_rnn$id,
  to = hidden_rnn$id
)

edges_rnn <- rbind(edges_rnn, recurrent_edges)

p3 <- plot_network(
  list(input_rnn, hidden_rnn, output_rnn),
  edges_rnn,
  "RN Recorrente"
)

# 4. DEEP NEURAL NETWORK
input_dnn <- create_layer(4, 1, "input")

h1_dnn <- create_layer(5, 2, "hidden")
h2_dnn <- create_layer(5, 3, "hidden")
h3_dnn <- create_layer(5, 4, "hidden")

output_dnn <- create_layer(2, 5, "output")

edges_dnn <- rbind(
  connect_layers(input_dnn, h1_dnn),
  connect_layers(h1_dnn, h2_dnn),
  connect_layers(h2_dnn, h3_dnn),
  connect_layers(h3_dnn, output_dnn)
)
...
# 5. AUTOENCODER
input_ae <- create_layer(5, 1, "input")
enc1_ae <- create_layer(4, 2, "hidden")
bottleneck_ae <- create_layer(2, 3, "memory")
dec1_ae <- create_layer(4, 4, "hidden")
output_ae <- create_layer(5, 5, "output")

edges_ae <- rbind(
  connect_layers(input_ae, enc1_ae),
  connect_layers(enc1_ae, bottleneck_ae),
  connect_layers(bottleneck_ae, dec1_ae),
  connect_layers(dec1_ae, output_ae)
)
...
# 6. CNN STYLE NETWORK
input_cnn <- create_layer(6, 1, "input")
conv1_cnn <- create_layer(5, 2, "hidden")
conv2_cnn <- create_layer(4, 3, "hidden")
dense_cnn <- create_layer(3, 4, "hidden")
output_cnn <- create_layer(2, 5, "output")
edges_cnn <- rbind(
  connect_layers(input_cnn, conv1_cnn),
  connect_layers(conv1_cnn, conv2_cnn),
  connect_layers(conv2_cnn, dense_cnn),
  connect_layers(dense_cnn, output_cnn)
)
...
# LEGENDA COMO PAINEL
legend_panel <- patchwork::wrap_elements(legend)
...
O pacote *neuralnet*[@neuralnet] fornece a função `plotnet` para visualizar redes neurais artificiais.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *neuralnet*[@neuralnet] fornece a função `neuralnet` para treinar redes neurais artificiais.
:::
...
```{r funcoes-ativacao, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.cap = "Gráficos das funções de ativação mais comuns.", fig.width=15, fig.height=4, out.width="100%"}
# grid comum
...
- Entropia cruzada categórica (*Categorical Cross-Entropy*, CCE \@ref(eq:loss-cce)): Essa função generaliza a entropia cruzada para múltiplas classes, penalizando previsões probabilísticas inconsistentes com a classe verdadeira.[@REF]
...
- Função hinge (*Support Vector Machines*, SVM \@ref(eq:loss-hinge)): Essa função busca maximizar a margem entre classes, penalizando classificações incorretas ou pouco confiantes em relação à fronteira de decisão.
...
#### Por que pesos são tratados como parâmetros do modelo?
...
### O que é o gradiente descendente (*gradient descent*)?
...
### O que é o gradiente descendente estocástico (*stochastic gradient descent*, SGD)?
...
### O que são *mini-batches* e como eles são utilizados no treinamento?
...
### O que é uma época (*epoch*) no treinamento de redes neurais?
...
### Como o número de épocas influencia ao risco de *overfitting*?
...
```{r perceptron-exemplo, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.cap = "Espaço de decisão de um perceptron (regressão logística).", fig.width=15, fig.height=8, out.width="100%"}
# Reprodutibilidade
set.seed(123)
...
```{r espaco-de-decisao, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.cap = "Comparação do espaço de decisão entre um modelo linear (regressão logística) e um modelo não linear (MLP).", fig.width=15, fig.height=8, out.width="100%"}
