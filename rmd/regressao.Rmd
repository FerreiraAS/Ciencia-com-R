...
```{r suposicoes, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', results = "asis", fig.fullwidth = TRUE, fig.cap = "Diagnóstico de regressão para avaliar suposições do modelo: linearidade, normalidade dos resíduos, homocedasticidade e alavancagem."}
...
# Função para título e subtítulo logo abaixo do título
add_title_with_sub <- function(main_title, subtitle, col_main = "black", col_sub = "red") {
  title(main = main_title, col.main = col_main)
  mtext(text = subtitle, side = 3, line = 0.5, col = col_sub, cex = 0.8)
}

# Layout 2x3 para os gráficos
par(mfrow = c(2, 3), mar = c(5, 4, 5, 2))  # margem superior maior para caber as 2 linhas
...
add_title_with_sub("Resíduos vs Ajustados", "--- Resíduo = 0")
...
qqnorm(resid(modelo),
       ...
       pch = 20,
       col = "darkgreen")
qqline(resid(modelo), col = "red", lty = 2)
add_title_with_sub("Normal Q-Q", "--- Distribuição Normal Teórica")

# Gráfico 3: Scale-Location
sqrt_resid <- sqrt(abs(scale(resid(modelo))))
plot(modelo$fitted.values,
     sqrt_resid,
     xlab = "Valores Ajustados",
     ...
     col = "orange")
abline(h = 0, col = "red", lty = 2)
add_title_with_sub("Scale-Location", "--- Referência visual")
...
add_title_with_sub("Distância de Cook", "--- Limite sugerido (4/n)")

# Gráfico 5: Observado vs Ajustado
limites <- range(c(dados$y, modelo$fitted.values))
plot(dados$y, modelo$fitted.values,
...
add_title_with_sub("Observado vs Ajustado", "--- y = x")
...
add_title_with_sub("Alavancagem", "--- 2 × média")
```
