...
trial <-
  tibble::tibble(
    Tratamento = factor(
      x = c(rep("A", n_A), rep("B", n_B)),
      levels = c("A","B")
    )
  ) %>%
  dplyr::mutate(
    mu     = dplyr::if_else(Tratamento == "A", 50, 55),
    desvio = dplyr::if_else(Tratamento == "A", 20, 5),
    Desfecho = stats::rnorm(n = dplyr::n(), mean = mu, sd = desvio)
  ) %>%
...
trial_t <- 
  ...
  gtsummary::tbl_summary(
    by = Tratamento,
    statistic = list(Desfecho ~ "{mean} ({sd})"),
    digits    = list(Desfecho ~ 2)
  ) %>%
  gtsummary::add_p(
    test = Desfecho ~ "t.test",
    test.args = list(gtsummary::all_tests("t.test") ~ list(var.equal = TRUE)),
    pvalue_fun = ~ gtsummary::style_pvalue(.x, digits = 3)
  ) %>%
  gtsummary::modify_footnote(
    update = p.value ~ "Teste t de Student (variâncias iguais)"
  ) %>%
  gtsummary::modify_header(p.value = "**P-valor**") %>%
  gtsummary::bold_labels() %>%
  ...
  gt::tab_source_note(
    base::paste0(
      ...
      base::round(
        base::as.numeric(
          psych::cohen.d(Desfecho ~ Tratamento, data = trial)$cohen.d[2]
        ),
        3
      )
    )
...
```{r t-student-pareado, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE}
# Reprodutibilidade
set.seed(1234)
...
trial <-
  tibble::tibble(
    ID = 1:n,
    Antes = stats::rnorm(n, mean = 50, sd = 10),
    Depois = Antes + stats::rnorm(n, mean = 5, sd = 8)
  )
...
trial_long <-
  ...
  dplyr::mutate(
    Momento = factor(Momento, levels = c("Antes","Depois"))
  )
...
trial_t <-
  ...
  gtsummary::add_p(
    test = Desfecho ~ "paired.t.test",
    group = ID,
    pvalue_fun = ~ gtsummary::style_pvalue(.x, digits = 3)
  ) %>%
  gtsummary::modify_footnote(
    update = p.value ~ "Teste t de Student (pareado)"
  ) %>%
  gtsummary::modify_header(p.value = "**P-valor**") %>%
  gtsummary::bold_labels() %>%
  gtsummary::modify_caption("Teste t de Student pareado") %>%
  gtsummary::as_gt() %>%
  gt::tab_source_note(
    paste0(
      "Tamanho do efeito (d de Cohen pareado): ",
      round(
        effectsize::cohens_d(trial$Depois, trial$Antes, paired = TRUE)$Cohens_d,
        3
      )
    )
  )
...
trial_t <- 
  ...
  gtsummary::add_p(
    test = Desfecho ~ "t.test",
    test.args = list(gtsummary::all_tests("t.test") ~ list(var.equal = FALSE)),
    pvalue_fun = ~ gtsummary::style_pvalue(.x, digits = 3)
  ) %>%
  gtsummary::modify_footnote(
    update = p.value ~ "Teste t de Welch (variâncias desiguais)"
  ) %>%
  gtsummary::modify_header(p.value = "**P-valor**") %>%
  gtsummary::bold_labels() %>%
...
eff_ind <- rstatix::wilcox_effsize(
  data = trial_ind,
  Desfecho ~ Tratamento,
  paired = FALSE
)
...
tbl_mw <- 
  ...
  gtsummary::modify_footnote(
    update = p.value ~ "Teste de Mann–Whitney (Wilcoxon rank-sum, amostras independentes)"
  ) %>%
  gtsummary::modify_header(p.value = "**P-valor**") %>%
  gtsummary::bold_labels() %>%
...
dados_pareados <-
  tibble::tibble(
    ID  = 1:m,
    Pre = rnorm(m, mean = 50, sd = 10),
    Pos = rnorm(m, mean = 55, sd = 10)
  ) %>%
  tidyr::pivot_longer(
    cols = c(Pre, Pos),
    names_to = "Tempo",
  ...
  dplyr::mutate(
    Tempo = factor(Tempo, levels = c("Pre","Pos"), labels = c("Pré","Pós"))
  ) %>%
...
tbl_wilcox <-
  ...
  gtsummary::tbl_summary(
    by = Tempo,
    include = Desfecho,
    statistic = list(Desfecho ~ "{median} [{p25}, {p75}]"),
    digits = list(Desfecho ~ 2)
  ) %>%
  gtsummary::add_p(
    test = Desfecho ~ "wilcox.test",
  ...
  gtsummary::modify_footnote(
    update = p.value ~ "Teste de Wilcoxon pareado (signed-rank, amostras dependentes)"
  ) %>%
  gtsummary::modify_header(p.value = "**P-valor**") %>%
  gtsummary::bold_labels() %>%
  gtsummary::modify_caption("Teste de Wilcoxon pareado (signed-rank)") %>%
  gtsummary::as_gt() %>%
  gt::tab_source_note(
    paste0(
      "Tamanho do efeito (r): ",
      round(as.numeric(eff_paired$effsize), 3)
    )
  )
...
tbl_anova <- 
  ...
  gtsummary::add_p(
    test = Desfecho ~ "oneway.test",
    pvalue_fun = ~ gtsummary::style_pvalue(.x, digits = 3)
  ) %>%
  gtsummary::modify_footnote(
    update = p.value ~ "ANOVA de um fator (variâncias iguais)"
  ) %>%
  gtsummary::modify_header(p.value = "**P-valor**") %>%
  gtsummary::bold_labels() %>%
...
tukey_tbl <-
  tibble::as_tibble(posthoc_tukey$Grupo, rownames = "Comparação") %>%
  dplyr::rename(
    diff   = diff,
    lwr    = lwr,
    upr    = upr,
...
```{r anova-welch, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE}
# Reprodutibilidade
base::set.seed(1234)
...
tbl_welch <-
  ...
  gtsummary::modify_footnote(
    update = p.value ~ "ANOVA de Welch (variâncias desiguais)"
  ) %>%
...
tbl_kw <- 
  ...
  gtsummary::modify_footnote(
    update = p.value ~ "Teste de Kruskal–Wallis (amostras independentes, variáveis assimétricas)"
  ) %>%
  gtsummary::modify_header(p.value = "**P-valor**") %>%
  gtsummary::bold_labels() %>%
