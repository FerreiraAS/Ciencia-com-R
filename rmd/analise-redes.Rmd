...
# **Análise de Redes** {#analise-redes}
...
-   Em uma matriz de incidência, as linhas representam um conjunto de entidades (nodos) e as colunas representam outro conjunto (arestas).[@leme2020]

- A presença ou ausência de uma relação entre as entidades é indicada por valores binários (0 ou 1) ou por pesos numéricos que refletem a intensidade da relação.[@leme2020]
...
```{r matriz-incidencia, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, tab.cap = "Matriz de incidência criada a partir de dados categóricos."}
# Exemplo dados categóricos
dados_cat <- data.frame(
  Sexo      = c("F", "M", "F", "F", "M"),
  Tabagismo = c("Sim", "Não", "Não", "Sim", "Não"),
...
O pacote *stats*[@stats] fornece a função `model.matrix` para criar uma matrix de incidência por expansão de variáveis indicadoras.
:::
...
```{r rede-exemplo, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Grafo de rede com nodos e arestas ponderadas."}
...
-   Um dos algoritmos mais utilizados é o Fruchterman–Reingold[@fruchterman1991], que simula forças de atração e repulsão entre os nodos, posicionando nodos mais fortemente associados mais próximos entre si.[@leme2020]
...
-   Em conjuntos de dados com muitas variáveis, redes podem se tornar densas e difíceis de interpretar.[@leme2020]

- Para contornar esse problema, utiliza-se penalização estatística, como o método *Least Absolute Shrinkage and Selection Operator* (LASSO), que elimina associações fracas, resultando em redes mais parcimoniosas e interpretáveis.[@leme2020]
...
```{r rede-metricas, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Grafo de rede com tamanho dos nodos proporcional à força."}
edges <- data.frame(
  from = c("A", "A", "B", "C", "D"),
  to   = c("B", "C", "C", "D", "A"),
  peso = c( 0.8, -0.5,  0.6, -0.7,  0.4)
...
```{r rede-metricas-tabela, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, tab.cap = "Tabela com principais métricas de centralidade dos nodos."}
edges <- data.frame(
  from = c("A", "A", "B", "C", "D"),
  to   = c("B", "C", "C", "D", "A"),
  peso = c( 0.8, -0.5,  0.6, -0.7,  0.4)
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *igraph*[@igraph] fornece a função `graph_from_incidence_matrix` para criar uma rede a partir de uma matriz de incidência.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *bootnet*[@bootnet] fornece a função `bootnet` para avaliação da estabilidade e precisão das redes por reamostragem.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *mgm*[@mgm] fornece a função `mgm` para estimação de modelos gráficos mistos.
:::
