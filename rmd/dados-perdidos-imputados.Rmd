...
```{r dados-perdidos, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, tab.cap = "Simulação de uma amostra (n=10) de um ensaio clínico aleatorizado (dados com perdas aleatórias)."}
# Reprodutibilidade
set.seed(123)
...
O pacote *base*[@base] fornece a função `is.na` para identificar que elementos de um objeto são dados perdidos.
:::
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *misty*[@misty] fornece a função `na.test` para executar o Little's Missing Completely at Random (MCAR) test[@little1988].
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *naniar*[@naniar] fornece a função `mcar_test` para executar o Little's Missing Completely at Random (MCAR) test[@little1988].
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *naniar*[@naniar] fornece a função `gg_miss_upset` para gerar o gráfico Upset para visualizar padrões de dados perdidos.
:::
...
-   Recomenda-se ampliar o tamanho da amostra com um percentual correspondente a tal estimativa (ex.: 10%), embora ainda não corrija potenciais vieses pela perda.[@Altman2007]

<br>

### Que estratégias utilizar na análise quando há dados perdidos?

-   A análise mais comum compreende apenas os 'casos completos', com exclusão de participantes com algum dado perdido nas variáveis do estudo. Em casos de grande quantidade de dados perdidos, pode-se perder muito poder estatístico (erro tipo II elevado).[@Altman2007]
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *base*[@base] fornece a função `na.omit` para remover dados perdidos de um objeto em um banco de dados.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *stats*[@stats] fornece a função `complete.cases` para identificar os casos completos em um banco de dados.
:::
...
- Métodos para tratamento de dados perdidos.[@Akl2015]

- Número de participantes com dados perdidos por variável.[@Akl2015]

- Motivos dos dados perdidos.[@Akl2015]

- Padrão de perda.[@Akl2015]

- Diferenças entre os participantes com e sem dados perdidos.[@Akl2015]

- Resultados de análises de sensibilidade.[@Akl2015]

- Implicações na interpretação do resultados.[@Akl2015]
...
-   Na ocorrência de dados perdidos, a imputação de dados pode ser uma alternativa para manter o erro tipo II estipulado no plano de análise.[@Altman2007]

<br>

### Quais são os métodos de imputação de dados?

- Imputação multivariada por equações encadeadas (*Multivariate Imputation by Chained Equations*, MICE)[@mice]

- Correspondência média preditiva (*Predictive Mean Matching*, PMM).[@rubin1986; @little1988a]
...
df$Painel_label <- factor(df$Método,
                    levels = c("Original",
                               "Imputação: Média",
                               "Imputação: Mediana",
                               "Imputação: Moda",
