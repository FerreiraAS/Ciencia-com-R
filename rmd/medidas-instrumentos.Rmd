...
# --- 1) Probabilidade de "acertar no ponto" é 0 em distribuição contínua ---
# Na simulação, P(X == theta) ≈ 0 (via contagem)
p_empirico_ponto <- mean(X == theta)  # deve ser 0 na prática
cat("Probabilidade empírica de X == θ (com N grande):", p_empirico_ponto, "\n")
...
g1 <- ggplot2::ggplot(df_plot, ggplot2::aes(x = X)) +
  ...
  ggplot2::labs(
    title = "Densidade de X = θ + erro (Normal)",
    subtitle = "Linha tracejada marca θ (ponto com probabilidade 0)",
    x = "Medida observada X", y = "Densidade"
  )

# (b) Curva P(|X-θ| ≤ ε) vs ε: analítica x empírica
df_eps <- data.frame(
...
g_mid <- ggplot2::ggplot(df_dens, ggplot2::aes(x = x, y = y)) +
  ...
  ggplot2::labs(
    ...
    subtitle = sprintf("ε = %.2f  |  P(|X-θ| ≤ ε) ≈ %.3f", eps_show, p_band),
    x = "Medida observada X", y = "Densidade"
  )
