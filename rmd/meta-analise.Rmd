...
-   A meta-análise é frequentemente considerada uma das formas mais robustas de evidência científica, pois permite a combinação quantitativa de múltiplos estudos, aumentando a precisão das estimativas de efeito e sintetizando o conhecimento disponível sobre determinada intervenção.[@Berlin2014]

- Por essas razões, a meta-análise deve ser interpretada com cautela, sendo considerada uma ferramenta poderosa de síntese, mas não uma garantia automática de evidência de alta qualidade.[@Berlin2014]
...
O pacote *metafor*[@metafor] fornece a função `forest` para criar figuras tipo *forest plot*.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *netmeta*[@netmeta] fornece a função `netmeta` para realizar meta-análise de rede usando método de grafo.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *gemtc*[@gemtc] fornece a função `mtc.model` para criar modelos de meta-análise de rede.
:::
...
ggplot2::ggplot(curve_data,
  ...
  ggplot2::labs(
    title = "Curvas Normais por Estudo: FE versus RE",
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *metafor*[@metafor] fornece a função `conv.fivenum` para converter mínimo/mediana/máximo ou Q1/mediana/Q2 em média e desvio-padrão.
:::
...
### Como avaliar a variação do tamanho de efeito?

- O intervalo de predição descreve a variação esperada do tamanho de efeito entre diferentes populações.[@Borenstein2022]

- Se o intervalo de predição não inclui a hipótese nula ($H_{0}$), espera-se que o tratamento apresente efeito na mesma direção em novas populações.[@Borenstein2022]

- O tamanho desse efeito pode variar entre as populações, mesmo quando a hipótese nula não é incluída no intervalo de predição.[@Borenstein2022]

- Se o intervalo de predição inclui a hipótese nula ($H_{0}$), o tratamento pode ser benéfico em algumas populações e ineficaz ou prejudicial em outras.[@Borenstein2022]

- Nessa situação, a estimativa pontual global torna-se menos informativa para a tomada de decisão.[@Borenstein2022]

- Recomenda-se investigar quais características das populações explicam essa variação do efeito do tratamento.[@Borenstein2022]

<br>

### Como avaliar a heterogeneidade entre os estudos?

-   A heterogeneidade --- variação não-aleatória --- no efeito do tratamento entre os estudos incluídos em uma meta-análise pode ser avaliada pelo $I^{2}$ \@ref(eq:i-sqr).[@Rucker2008; @Borenstein2022]
...
-   $I^{2}$ mede a proporção da variância total que pode ser atribuída à heterogeneidade entre os estudos incluídos.[@Rucker2008]

-   $I^{2}$ não depende da quantidade de estudos incluídos na meta-análise. Entretanto, $I^{2}$ aumenta com a quantidade de participantes incluídos nos estudos meta-analisados.[@Rucker2008]
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *psychmeta*[@psychmeta] fornece a função `ma_d` para meta-analisar valores *d*.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *psychmeta*[@psychmeta] fornece a função `ma_r` para meta-analisar correlações.
:::
...
```{r crosshair, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Gráfico de cruzes em espaço de curva característica de operação do receptor (ROC) para 15 estudos simulados de desempenho diagnóstico."}
# Reprodutibilidade
set.seed(123)
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *mada*[@mada] fornece a função `crosshair` para criar um gráfico *crosshair*[@phillips2010] a partir de dados de verdadeiro-positivo, falso-positivo, verdadeiro-negativo e verdadeiro-positivo de tabelas de confusão 2x2.
:::
...
-   É um gráfico de dispersão que relaciona a estimativa de efeito de cada estudo com uma medida de seu tamanho ou precisão (por exemplo, erro-padrão no eixo vertical, em escala invertida).[@sterne2011]

- Em condições ideais (ausência de viés e heterogeneidade), os estudos se distribuem de forma simétrica, formando um “funil invertido”.[@sterne2011]

<br>

```{r ma-funnel-plot, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.cap = "Gráficos de funil simulados com baixa e alta heterogeneidade.", fig.width=15, fig.height=4, out.width="100%"}
# Reprodutibilidade
set.seed(123)
...
-   O método *trim and fill* “apara” os estudos que causam assimetria no funnel plot, estima o número de estudos possivelmente ausentes (não publicados) e “preenche” o gráfico com esses estudos simulados, recalculando o efeito combinado.[@duval2000]
...
## Testes de assimetria do *funnel plot*
...
-   Essa abordagem reduz a correlação entre $ln(OR)$ e seu $SE$, resultando em taxas de erro tipo I mais adequadas (\~10%) independentemente do tamanho de efeito, número de estudos ou heterogeneidade.[@peters2006]
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *psychmeta*[@psychmeta] fornece a função `plot_funnel` para criar figuras tipo *funnel plot*.
:::
...
-   *The PRISMA 2020 statement: An updated guideline for reporting systematic reviews*.[@page2021]
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *metagear*[@metagear] fornece a função `plot_PRISMA` para gerar o fluxograma de uma revisão sistemática de acordo com o *Preferred Reporting Items for Systematic Reviews and Meta-Analyses*[@Moher2015].
:::
