...
-  A regressão pode ser classificada em univariada (simples), multivariável (múltipla) e multivariada, dependendo do número de variáveis dependentes e independentes envolvidas no modelo.[@Hidalgo2013]

<br>

```{r regressao-tipos, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Tipos de regressão: univariada (simples), multivariável (múltipla) e multivariada."}
grid::grid.newpage()

# Funções auxiliares
caixa <- function(x, y, w, h, texto, fill = "white") {
  grid::grid.roundrect(
    x = x,
    y = y,
    width = w,
...
seta <- function(x0, y0, x1, y1, col = "black") {
  grid::grid.lines(
...
# UNIVARIADO
caixa(.08, .45, .08, .08, expression(X[1]))
elipse(.23, .45, .10, .10, expression(Y[1]))
seta(.12, .45, .18, .45)

# MULTIVARIÁVEL

ys <- c(.60, .48, .36, .24)
preditores <- c(expression(X[1]),
                expression(X[2]),
                expression(X[3]),
                expression(X[4]))

for (i in 1:4) {
  caixa(.41, ys[i], .09, .07, preditores[i])
  seta(.455, ys[i], .56, .45)
}
elipse(.60, .45, .11, .11, expression(Y[1]))

# MULTIVARIADO

for (i in 1:4) {
  caixa(.74, ys[i], .09, .07, preditores[i])
}

yr <- c(.60, .45, .30)
respostas  <- c(expression(Y[1]), expression(Y[2]), expression(Y[3]))
...
grid::grid.lines(
  x = grid::unit(c(.90, .90), "npc"),
  y = grid::unit(c(.56, .49), "npc"),
...
rodape(.165, "1 X → 1 Y", "black")
rodape(.495, "Vários X → 1 Y", "black")
rodape(.83, "Vários X → Vários Y", "black")
```
