...
ggplot2::ggplot(data_ext, ggplot2::aes(x = X, y = Y)) +
  ...
  ggplot2::scale_shape_manual(values = c(`FALSE` = 16, `TRUE` = 17), guide = "none") +  # <- legenda removida aqui
...
ggplot2::ggplot(data_ext, ggplot2::aes(x = leverage, y = resid_std)) +
  ...
  ggplot2::scale_shape_manual(values = c(`FALSE` = 16, `TRUE` = 17),
                              guide = "none") +      # remove legenda de shape
  ggplot2::scale_size_continuous(range = c(2, 8), guide = "none") +  # remove legenda de size
  ggplot2::scale_linetype_manual(values = c("0.5" = "longdash", "1" = "solid"),
                                 name = "Curvas de Cook") +

  ggplot2::labs(
    title = "Alavancage vs Resíduos Padronizados",
...
```{r regressao-linear-outliers, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Regressão linear com valores discrepantes."}
# Reprodutibilidade
set.seed(123)
