...
p_st <- base +
  ...
  ggplot2::labs(
    title = "Sturges",
    subtitle = sprintf("binwidth = %.4f", stg$k, bw_st)
  )
...
- Intervalo de confiança para variância conhecida \@ref(eq:ic-known) e desconhecida \@ref(eq:ic-unknown): Captura a média populacional correspondente ao nível de significância $\alpha$ pré-estabelecido [@kanji2006; @Cumming2007; @krzywinski2013; @Ali2016].
...
resumo <- df |>
  ...
  dplyr::summarise(
    ...
    # kurtose "total" (Pearson)
    excess  = kurt - 3                       # excess kurtosis
  )
...
-   *Median Absolute Deviation* (MAD) \@ref(eq:mad), com correção 1,483 para normalidade, com $~50%$ de ponto de quebra.[@rousseeuw2011]
...
- Primeiro quartil das diferenças pareadas ($Qn$) \@ref(eq:qn), com $~50%$ de ponto de quebra.[@rousseeuw2011]
...
-   *Median Absolute Deviation* (MAD) em vez do desvio-padrão $\sigma$, que pode ser escalonado por 1,483 para comparabilidade.[@daszykowski2007]
