...
Os pacotes *ggplot2*[@ggplot2], *plotly*[@plotly] e *corrplot*[@corrplot] fornecem diversas funções para construção de gráficos tais como `ggplot`, `plot_ly` e `corrplot` respectivamente.
:::
...
- *Checklist*: (1) Mensagem está explícita? (2) Geometria adequada e dados visíveis? (3) Incerteza correta e rotulada? (4) Cores informativas e acessíveis? (5) Escalas comparáveis? (6) Legenda autossuficiente? (7) Diferença clara entre dados e modelos? (8) Arquivo exportado na resolução/tamanho exigidos?[@midway2020]

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *ggsci*[@ggsci] fornece palhetas de cores tais como `pal_lancet`, `pal_nejm` e `pal_npg` inspiradas em publicações científicas para uso em gráficos.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *grDevices*[@grDevices] fornece a função `dev.new` para controlar diversos aspectos do gráfico, tais como tamanho e resolução.
:::

<br>

::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *tiff*[@tiff] fornece a função `writeTIFF` para exportar gráficos em formato TIFF.
:::
...
file.name <- "fluxograma-prisma.png"
...
png_file  <- file.path(out_path, "fluxograma-prisma.png")
...
# delete HTML file after conversion to PNG
invisible(file.remove(html_file, showWarnings = FALSE))
...
```{r fluxograma-consort, echo = FALSE, warning = FALSE, message = FALSE, appendix = TRUE, fig.align = 'center', layout='Blank', ph=officer::ph_location_fullsize(), results = "asis", fig.fullwidth = TRUE, fig.cap = "Fluxograma baseado no *Consolidated Standards of Reporting Trials* (CONSORT)."}
# Reprodutibilidade
set.seed(123)
...
dados <- data.frame(
  ...
  arm = NA_character_,
  arm3 = NA_character_,
  subjid_notdosed = NA_character_,
  followup = NA_character_,
  lost_followup = NA_character_,
  assessed = NA_character_,
...
inc <- which(is.na(dados$exclusion))

dados$arm[inc] <-
  sample(
    c("Controle", "Intervenção"),
    length(inc),
    replace = TRUE
  )
...
nao_tratados <- sample(inc, 12)

dados$arm3[inc] <- dados$arm[inc]

dados$subjid_notdosed[nao_tratados] <-
  sample(c(
...
tratados <- setdiff(inc, nao_tratados)

dados$followup[tratados] <- "Seguimento completo"

perdidos <- sample(tratados, 10)

dados$lost_followup[perdidos] <-
  sample(c(
    "Perda de contato",
    "Mudança de cidade",
    "Desistência"
...
avaliados <- setdiff(tratados, perdidos)

dados$assessed[avaliados] <- "Avaliado"

nao_avaliados <- sample(avaliados, 8)

dados$no_value[nao_avaliados] <-
  sample(c(
    "Exame ausente",
    "Dados incompletos"
  ),
  length(nao_avaliados),
  replace = TRUE)

## mITT

incluidos <- setdiff(avaliados, nao_avaliados)

dados$mitt[incluidos] <- "Incluído"

## Fluxograma

p <- consort::consort_plot(
  data = dados,
  orders = list(
    c(trialno = "População"),
    c(exclusion = "Excluídos"),
    c(arm = "Randomizados"),
    c(
      arm3 = "",
      subjid_notdosed = "Não tratados"
    ),
    c(
      followup = "Seguimento",
      lost_followup = "Perdas"
    ),
    c(assessed = "Avaliados"),
    c(no_value = "Não avaliados"),
    c(mitt = "Incluídos na análise")
  ),
...
::: {.infobox .Rlogo data-latex="{images/Rlogo}"}
O pacote *PRISMA2020*[@PRISMA2020] fornece a função `PRISMA_flowdiagram` para elaboração do fluxograma de revisões sistemáticas no formato padrão.
:::
