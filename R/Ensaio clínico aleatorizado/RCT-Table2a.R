TABLE.2a <-
  function(dataset,
    ...
    if (missing == "multiple.imputation") {
      ...
      if (!is.null(covariate)) {
        COVARIATE_M[which(is.na(COVARIATE_M))] <-
          mean(COVARIATE_M[which(!is.na(COVARIATE_M))])
      }
    }
    ...
    if (missing != "multiple.imputation") {
      ...
      if (!is.null(covariate)) {
        ...
        mod1.aov <-
          quiet(mi.anova(imp, formula = "OUTCOME_M ~ TIME_M * GROUP_M + COVARIATE_M"))
      } else {
        mod1 <-
          with(data = imp,
               lme(
        ...
        mod1.aov <-
          quiet(mi.anova(imp, formula = "OUTCOME_M ~ TIME_M * GROUP_M"))
      }
    ...
    for (i in 2:3) {
      ...
      model.res[i - 1,] <-
        paste(
          ...
          ",",
          format(round(mod1.aov[i, 2], digits = 0),
                 nsmall = 0),
          ") = ",
    ...
    mult.within <-
      summary(pairs(emmeans(mod1, ~ TIME_M |
    ...
    for (i in 1:nlevels(bw.factor)) {
      ...
      low.ci <-
        -round(as.numeric(group.data[, 7]), digits = n.digits)
      upp.ci <-
        -round(as.numeric(group.data[, 6]), digits = n.digits)
      wt <-
        cbind(wt, t(paste(
      ...
      for (k in 1:(length(wt.labels) - 1)) {
        ...
        ifelse(p.values[k] > 0.001 &
                 p.values[k] < alpha,
               wt.pvalues <-
                 c(wt.pvalues, paste(format(
                   round(p.values[k], digits = 3), nsmall = 3
                 ),
                 "*", sep = "")),
               "")
        ifelse(p.values[k] > alpha, wt.pvalues <-
                 c(wt.pvalues, format(
                   round(p.values[k], digits = 3), nsmall = 3
                 )), "")
      }
    ...
    for (i in 2:(length(wt.labels))) {
      ...
      if (missing != "multiple.imputation") {
        if (!is.null(covariate)) {
          ...
          mod2 <-
            lme(
              CHANGE_M ~ bw.factor + BASELINE_M + COVARIATE_M,
              random = ~ 1 | ID,
              data = df
            )
        } else {
          ...
          mod2 <-
            lme(CHANGE_M ~ bw.factor + BASELINE_M,
                random = ~ 1 | ID,
                data = df)
        }
        mod2.sum <-
          summary(glht(mod2, linfct = mcp(bw.factor = "Tukey")), test = adjusted("holm"))
        ...
        mod2 <-
          with(data = implist, lm(CHANGE_M ~ bw.factor + BASELINE_M, random = ~
                                    1 | ID))
        fit2.pairwise <-
          lapply(mod2, glht, linfct = mcp(bw.factor = "Tukey"))
        fit2.pairwise <- as.mitml.result(fit2.pairwise)
        mod2.sum <- testEstimates(fit2.pairwise)
        names <-
          rownames(confint.mitml.testEstimates(mod2.sum, level = 1 - alpha))
        estimate <-
          round(unlist(mod2.sum)$estimates1, digits = n.digits)
        low.ci <-
          round(confint.mitml.testEstimates(mod2.sum, level = 1 - alpha)[1, 1],
                digits = n.digits)
        upp.ci <-
          round(confint.mitml.testEstimates(mod2.sum, level = 1 - alpha)[1, 2],
                digits = n.digits)
